...
def main():
    ...
    # A report left over from a previous run would otherwise be read as this run's result.
    ...
    # With --check_format=json lls always writes the report, an empty list when the scripts are clean.
    # A missing report means lls never got as far as checking anything.
    if not os.path.exists(log_file):
        print_lls(f"No report at {log_file}, lua-language-server exited with code {result.returncode}")
        return 1
