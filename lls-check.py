...
def main():
    ...
    check_level = "Error" # TODO(Gerark) Temporary checking only for errors now but will soon change to warning after fixing all of the scripts in future commits
    lua_language_server_exe_path = sys.argv[1]
    scripts_folder = sys.argv[2]
    log_output = sys.argv[3]
    print_lls(log_output)
    configuration_path = sys.argv[4]
    print_lls(configuration_path)
...
def parse_log(data):
    ...
    for file, logs in data.items():
        errors = []
        for log in logs:
            line = log['range']['start']['line']
            character = log['range']['start']['character']
