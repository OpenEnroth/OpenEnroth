...
CI also runs `clang-tidy` with the checks listed in `.clang-tidy`, and any finding fails the build. Running it locally needs clang-tidy 22 with `run-clang-tidy` next to it on PATH, and distro packages are usually older. CI installs the `clang-tidy` package from PyPI, which ships both, so install that the way you install other python command line tools, with pipx or a virtualenv rather than into the system python. Reconfigure so that cmake picks it up, then build the `check_tidy` target, which needs a compile commands database and so is only offered by the ninja and makefile generators.
...
* We generally refrain from using namespaces because OpenEnroth is a relatively small codebase, and we don't need the measures advocated by the Google style guide to prevent name clashes.
  ...
  * Use `static` for functions and variables that are private to a `.cpp` file, and not an anonymous namespace. This is the opposite of what the Google style guide advises, but it reads better, and it's what the codebase does.
