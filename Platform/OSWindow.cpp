...
unsigned int OSWindow::GetWidth() const {
    RECT rc;
    GetClientRect((HWND)api_handle, &rc);
    return rc.right - rc.left;
}
unsigned int OSWindow::GetHeight() const {
    RECT rc;
    GetClientRect((HWND)api_handle, &rc);
...
bool OSWindow::WinApiMessageProc(int msg, int wparam, void *lparam,
                                 void **result) {
    switch (msg) {
        case WM_KEYUP: {
...
bool OSWindow::Initialize(const wchar_t *title, int window_width,
                          int window_height) {
    WNDCLASSEXW wcxw;
