...
void *WinOSWindow::GetApiHandle() const {
    return api_handle;
}

int WinOSWindow::GetX() const {
    RECT rc;
    GetWindowRect((HWND)api_handle, &rc);
    return rc.left;
}
int WinOSWindow::GetY() const {
    RECT rc;
    GetWindowRect((HWND)api_handle, &rc);
    return rc.top;
}
unsigned int WinOSWindow::GetWidth() const {
    RECT rc;
    GetClientRect((HWND)api_handle, &rc);
    return rc.right - rc.left;
}
unsigned int WinOSWindow::GetHeight() const {
    RECT rc;
    GetClientRect((HWND)api_handle, &rc);
    return rc.bottom - rc.top;
}
bool WinOSWindow::Focused() { return GetFocus() == (HWND)api_handle; }

Point WinOSWindow::TransformCursorPos(Point &pt) const {
...
bool WinOSWindow::OnMouseLeftClick(int x, int y) {
...
bool WinOSWindow::OnMouseRightClick(int x, int y) {
    pMediaPlayer->StopMovie();
...
bool WinOSWindow::WinApiMessageProc(int msg, int wparam, void *lparam,
                                 void **result) {
    switch (msg) {
        case WM_KEYUP: {
        ...
        case WM_KEYDOWN:
            ...
            if (!pArcomageGame->bGameInProgress) {
                ...
                if (wparam > VK_DOWN) {
                    ...
                    SendMessageW((HWND)this->GetApiHandle(), WM_COMMAND, 104,
                                 0);
                    return 0;
                }
        ...
        case WM_PAINT:
            if (!api_handle || !render) return false;

            if (!GetUpdateRect((HWND)this->GetApiHandle(), 0,
                               0))  // || !dword_4E98BC_bApplicationActive &&
                                    // !render->bWindowMode )
                return *result = 0, true;
...
bool WinOSWindow::Initialize(const wchar_t *title, int window_width,
                          int window_height) {
    WNDCLASSEXW wcxw;
    ...
    wcxw.hbrBackground =
        (HBRUSH)GetStockObject(BLACK_BRUSH);  // (COLOR_BACKGROUND + 1);
    wcxw.lpszMenuName = nullptr;
    wcxw.lpszClassName = L"M&MTrilogy";
    if (!RegisterClassExW(&wcxw)) return false;

    api_handle = CreateWindowExW(
        NULL, wcxw.lpszClassName, title, 0,
        OS_GetAppInt("window X",
                     (GetSystemMetrics(SM_CXSCREEN) - window_width) / 2),
        OS_GetAppInt("window Y",
...
OSWindow *WinOSWindow::Create(const wchar_t *title, int window_width,
                           int window_height) {
...
void *WinOSWindow::WinApiMsgRouter(void *hwnd, int msg, int wparam, void *lparam) {
    if (msg == WM_NCCREATE) {
    ...
    if (window &&
        window->GetApiHandle() == hwnd) {  // Uninitialized memory access
        void *result;
        if (window->WinApiMessageProc(
                msg, wparam, lparam,
                &result))  // Unhandled application exception
            return result;
    }
    return (void *)DefWindowProcW((HWND)hwnd, msg, (WPARAM)wparam,
                                  (LPARAM)lparam);
}

void WinOSWindow::Show() {
    ShowWindow((HWND)this->GetApiHandle(), SW_SHOWNORMAL);
    UpdateWindow((HWND)this->GetApiHandle());
}

void WinOSWindow::SetCursor(const char *cursor_name) {
    POINT cursor_pos;
    ...
    if (!strcmp(cursor_name, "MICON1")) {
        ...
        SetClassLongPtrW((HWND)this->GetApiHandle(), GCLP_HCURSOR,
...
void WinOSWindow::SetFullscreenMode() {
...
void WinOSWindow::SetWindowedMode(int new_window_width, int new_window_height) {
    auto hwnd = (HWND)this->GetApiHandle();
...
#ifdef _DEBUG
    if (!GetMenu(hwnd)) window_total_height += GetSystemMetrics(SM_CYMENU);
#endif

    MoveWindow(hwnd, rcWindowPos.left, rcWindowPos.top, window_total_width,
...
void *WinOSWindow::CreateDebugMenuPanel() {
    HMENU menu = CreateMenu();
    {
                    ...
                    L"&Debug");
        {
                        ...
                        (UINT_PTR)debug_party, L"&Party");
            {
                ...
                AppendMenuW(debug_party, MF_ENABLED | MF_STRING | MF_POPUP,
                            ...
                            L"Set Condition");
                {
                    AppendMenuW(debug_party_setconditions,
                                MF_ENABLED | MF_STRING, 40044, L"Afraid");
                    AppendMenuW(debug_party_setconditions,
                                MF_ENABLED | MF_STRING, 40043, L"Asleep");
                    AppendMenuW(debug_party_setconditions,
                                MF_ENABLED | MF_STRING, 40037, L"Curse");
                    AppendMenuW(debug_party_setconditions,
                                MF_ENABLED | MF_STRING, 40036, L"Disease Weak");
                    AppendMenuW(debug_party_setconditions,
                                MF_ENABLED | MF_STRING, 40035,
                                L"Disease Medium");
                    AppendMenuW(debug_party_setconditions,
                                MF_ENABLED | MF_STRING, 40034,
                                L"Disease Severe");
                    AppendMenuW(debug_party_setconditions,
                                MF_ENABLED | MF_STRING, 40041, L"Dead");
                    AppendMenuW(debug_party_setconditions,
                                MF_ENABLED | MF_STRING, 40039, L"Drunk");
                    AppendMenuW(debug_party_setconditions,
                                MF_ENABLED | MF_STRING, 40042, L"Eradicated");
                    AppendMenuW(debug_party_setconditions,
                                MF_ENABLED | MF_STRING, 40038, L"Insane");
                    AppendMenuW(debug_party_setconditions,
                                MF_ENABLED | MF_STRING, 40045, L"Paralyzed");
                    AppendMenuW(debug_party_setconditions,
                                MF_ENABLED | MF_STRING, 40033, L"Poison Weak");
                    AppendMenuW(debug_party_setconditions,
                                MF_ENABLED | MF_STRING, 40032,
                                L"Poison Medium");
                    AppendMenuW(debug_party_setconditions,
                                MF_ENABLED | MF_STRING, 40031,
                                L"Poison Severe");
                    AppendMenuW(debug_party_setconditions,
                                MF_ENABLED | MF_STRING, 40029, L"&Stone");
                    AppendMenuW(debug_party_setconditions,
                                MF_ENABLED | MF_STRING, 40040, L"Unconscious");
                    AppendMenuW(debug_party_setconditions,
                                MF_ENABLED | MF_STRING, 40030, L"Weak");
                    AppendMenuW(debug_party_setconditions,
                                MF_ENABLED | MF_STRING, 40073, L"Zombie");
                    AppendMenuW(debug_party_setconditions,
                                MF_ENABLED | MF_STRING, 40046, L"Good");
                }

                AppendMenuW(debug_party, MF_ENABLED | MF_STRING, 40006,
                            L"Set Food (20)");

                HMENU debug_party_alignment = CreatePopupMenu();
                AppendMenuW(debug_party, MF_ENABLED | MF_STRING | MF_POPUP,
                            (UINT_PTR)debug_party_alignment, L"Alignment");
                {
                    AppendMenuW(debug_party_alignment, MF_ENABLED | MF_STRING,
                                40062, L"Good");
                    AppendMenuW(debug_party_alignment,
                                MF_ENABLED | MF_STRING | MF_CHECKED, 40063,
                                L"Neutral");
                    AppendMenuW(debug_party_alignment, MF_ENABLED | MF_STRING,
                                40064, L"Evil");
                }
            ...
            AppendMenuW(debug, MF_ENABLED | MF_STRING | MF_POPUP,
                        (UINT_PTR)debug_time, L"&Time");
            {
                AppendMenuW(debug_time, MF_ENABLED | MF_STRING, 40009,
                            L"Add 1 Day");
                AppendMenuW(debug_time, MF_ENABLED | MF_STRING, 40010,
                            L"Add 1 Week");
                AppendMenuW(debug_time, MF_ENABLED | MF_STRING, 40011,
                            L"Add 1 Month");
                AppendMenuW(debug_time, MF_ENABLED | MF_STRING, 40012,
                            L"Add 1 Year");
            }
            ...
            {
                AppendMenuW(debug_items, MF_ENABLED | MF_STRING, 40015,
                            L"Generate level &1 item");
                AppendMenuW(debug_items, MF_ENABLED | MF_STRING, 40016,
                            L"Generate level &2 item");
                AppendMenuW(debug_items, MF_ENABLED | MF_STRING, 40017,
                            L"Generate level &3 item");
                AppendMenuW(debug_items, MF_ENABLED | MF_STRING, 40018,
                            L"Generate level &4 item");
                AppendMenuW(debug_items, MF_ENABLED | MF_STRING, 40019,
                            L"Generate level &5 item");
                AppendMenuW(debug_items, MF_ENABLED | MF_STRING, 40020,
                            L"Generate level &6 item");
                AppendMenuW(debug_items, MF_ENABLED | MF_STRING, 40061,
                            L"Generate special item");
            }
                        ...
                        (UINT_PTR)debug_graphics, L"&Graphics");
            {
                ...
                {
                    AppendMenuW(lights_off, MF_ENABLED | MF_STRING, 40123,
                                L"Lights on");
                    AppendMenuW(lights_off, MF_ENABLED | MF_STRING, 40124,
                                L"Lights off");
                }
                // AppendMenuW(debug_graphics, MF_ENABLED | MF_STRING, 40025,
                // L"Debug Lights");
                HMENU debug_lights = CreatePopupMenu();
                AppendMenuW(debug_graphics, MF_ENABLED | MF_STRING | MF_POPUP,
                            (UINT_PTR)debug_lights, L"Debug Lights");  // 40104
                {
                    AppendMenuW(debug_lights, MF_ENABLED | MF_STRING, 40125,
                                L"Debug lights on");
                    AppendMenuW(debug_lights, MF_ENABLED | MF_STRING, 40126,
                                L"Debug lights off");
                }
                ...
                {
                    AppendMenuW(debug_terrain, MF_ENABLED | MF_STRING, 40127,
                                L"Debug Terrain on");
                    AppendMenuW(debug_terrain, MF_ENABLED | MF_STRING, 40128,
                                L"Debug Terrain off");
                }
                AppendMenuW(debug_graphics, MF_ENABLED | MF_STRING | MF_GRAYED,
                            40023, L"Lighting Mode");
                AppendMenuW(debug_graphics, MF_ENABLED | MF_STRING | MF_GRAYED,
                            40024, L"Lighting Geometry");

                // AppendMenuW(debug_graphics, MF_ENABLED | MF_STRING/* |
                // MF_GRAYED*/, 40123, L"Lights Off");
                AppendMenuW(debug_graphics, MF_ENABLED | MF_STRING | MF_GRAYED,
                            40105, L"Colored Lights");
                AppendMenuW(debug_graphics, MF_ENABLED | MF_STRING | MF_GRAYED,
                            40101, L"Debug Decals");
                AppendMenuW(debug_graphics, MF_ENABLED | MF_STRING | MF_GRAYED,
                            40027, L"HWID Portals");
                AppendMenuW(debug_graphics, MF_ENABLED | MF_STRING | MF_GRAYED,
                            40047, L"SWID Portals");
                AppendMenuW(debug_graphics, MF_ENABLED | MF_STRING | MF_GRAYED,
                            40051, L"OD Frustum");
                AppendMenuW(debug_graphics, MF_ENABLED | MF_STRING | MF_GRAYED,
                            40054, L"SWOD Constant Redraw");
                AppendMenuW(debug_graphics, MF_ENABLED | MF_STRING | MF_GRAYED,
                            40055, L"SWOD Lit Rasterizer");
                AppendMenuW(debug_graphics, MF_ENABLED | MF_STRING | MF_GRAYED,
                            40056, L"Party Light off");
                AppendMenuW(debug_graphics, MF_ENABLED | MF_STRING | MF_GRAYED,
                            40060, L"SWOD Nice Lighting off");
                AppendMenuW(debug_graphics, MF_ENABLED | MF_STRING | MF_GRAYED,
                            40067, L"HWOD Additive Fog Lights");
                AppendMenuW(debug_graphics, MF_ENABLED | MF_STRING | MF_GRAYED,
                            40072, L"HWID Nice Lighting");
                AppendMenuW(debug_graphics, MF_ENABLED | MF_STRING | MF_GRAYED,
                            40048, L"Wireframe");
                AppendMenuW(debug_graphics, MF_ENABLED | MF_STRING | MF_GRAYED,
                            40049, L"Fog");
            ...
            {
                AppendMenuW(debug_misc, MF_ENABLED | MF_STRING | MF_GRAYED,
                            40066, L"Object Viewcone Culling");
                AppendMenuW(debug_misc, MF_ENABLED | MF_STRING | MF_GRAYED,
                            40068, L"Red Tint");
                AppendMenuW(debug_misc, MF_ENABLED | MF_STRING | MF_GRAYED,
                            40071, L"Display Secrets");
                AppendMenuW(debug_misc, MF_ENABLED | MF_STRING | MF_GRAYED,
                            40102, L"Massive Bloodsplat");
                AppendMenuW(debug_misc, MF_ENABLED | MF_STRING | MF_GRAYED,
                            40103, L"Underwater Gravity");
            }
            ...
            {
                AppendMenuW(debug_eax, MF_ENABLED | MF_STRING | MF_GRAYED,
                            40074, L"NONE");
                AppendMenuW(debug_eax, MF_ENABLED | MF_STRING | MF_GRAYED,
                            40075, L"GENERIC");
                AppendMenuW(debug_eax, MF_ENABLED | MF_STRING | MF_GRAYED,
                ...
                AppendMenuW(debug_eax, MF_ENABLED | MF_STRING | MF_GRAYED,
                            40085, L"HANGAR");
                AppendMenuW(debug_eax, MF_ENABLED | MF_STRING | MF_GRAYED,
                            40086, L"CARPETEDHALLWAY");
                AppendMenuW(debug_eax, MF_ENABLED | MF_STRING | MF_GRAYED,
                            40087, L"HALLWAY");
                AppendMenuW(debug_eax, MF_ENABLED | MF_STRING | MF_GRAYED,
                            40088, L"STONECORRIDOR");
                AppendMenuW(debug_eax, MF_ENABLED | MF_STRING | MF_GRAYED,
                            40089, L"ALLEY");
                AppendMenuW(debug_eax, MF_ENABLED | MF_STRING | MF_GRAYED,
                            40090, L"FOREST");
                    ...
                    L"&Other");
        {
            ...
            {
                AppendMenuW(other_wizard_eye, MF_ENABLED | MF_STRING, 40101,
                            L"Wizard eye on");
                AppendMenuW(other_wizard_eye, MF_ENABLED | MF_STRING, 40102,
                            L"Wizard eye off");
            }
            ...
            {
                AppendMenuW(other_new_draw_object_dist, MF_ENABLED | MF_STRING,
                            40103, L"New draw object dist on");
                AppendMenuW(other_new_draw_object_dist, MF_ENABLED | MF_STRING,
                            40104, L"New draw object dist off");
            }
            HMENU other_change_seasons = CreatePopupMenu();
            AppendMenuW(other, MF_ENABLED | MF_STRING | MF_POPUP,
                        (UINT_PTR)other_change_seasons, L"Change seasons");
            {
                AppendMenuW(other_change_seasons, MF_ENABLED | MF_STRING, 40105,
                            L"Change seasons on");
                AppendMenuW(other_change_seasons, MF_ENABLED | MF_STRING, 40106,
                            L"Change seasons off");
            }
            HMENU other_all_magic = CreatePopupMenu();
            AppendMenuW(other, MF_ENABLED | MF_STRING | MF_POPUP,
                        (UINT_PTR)other_all_magic, L"All magic");
            {
                AppendMenuW(other_all_magic, MF_ENABLED | MF_STRING, 40107,
                            L"All magic on");
                AppendMenuW(other_all_magic, MF_ENABLED | MF_STRING, 40108,
                            L"All magic off");
            }
            ...
            AppendMenuW(other, MF_ENABLED | MF_STRING | MF_POPUP,
                        ...
                        L"Debug information");
            {
                AppendMenuW(other_debug_information, MF_ENABLED | MF_STRING,
                            40109, L"Debug information on");
                AppendMenuW(other_debug_information, MF_ENABLED | MF_STRING,
                            40110, L"Debug information off");
            }
            HMENU other_show_picked_face = CreatePopupMenu();
            AppendMenuW(other, MF_ENABLED | MF_STRING | MF_POPUP,
                        (UINT_PTR)other_show_picked_face, L"Show picked face");
            {
                AppendMenuW(other_show_picked_face, MF_ENABLED | MF_STRING,
                            40111, L"Show picked face on");
                AppendMenuW(other_show_picked_face, MF_ENABLED | MF_STRING,
                            40112, L"Show picked face off");
            }
            ...
            {
                AppendMenuW(other_draw_portals_loops, MF_ENABLED | MF_STRING,
                            40113, L"Draw portals loops on");
                AppendMenuW(other_draw_portals_loops, MF_ENABLED | MF_STRING,
                            40114, L"Draw portals loops off");
            }
            HMENU other_new_speed = CreatePopupMenu();
            AppendMenuW(other, MF_ENABLED | MF_STRING | MF_POPUP,
                        (UINT_PTR)other_new_speed, L"New_speed");
            {
                AppendMenuW(other_new_speed, MF_ENABLED | MF_STRING, 40115,
                            L"New_speed on");
                AppendMenuW(other_new_speed, MF_ENABLED | MF_STRING, 40116,
                            L"New_speed off");
            }
            ...
            {
                AppendMenuW(other_snow, MF_ENABLED | MF_STRING, 40117,
                            L"Snowfall on");
                AppendMenuW(other_snow, MF_ENABLED | MF_STRING, 40118,
                            L"Snowfall off");
            }
            ...
            {
                AppendMenuW(other_draw_terrain_dist_mist,
                            MF_ENABLED | MF_STRING, 40119,
                            L"New draw terrain distance on");
                AppendMenuW(other_draw_terrain_dist_mist,
                            MF_ENABLED | MF_STRING, 40120,
                            L"New draw terrain distance off");
            }
        }
    }
    return menu;
...
bool WinOSWindow::OnOSMenu(int item_id) {
    switch (item_id) {
        default:
            return false;
        ...
        case 40032:
            ...
            break;
        case 40033:
            pPlayers[uActiveCharacter]->SetPoisonWeak(pParty->GetPlayingTime());
            break;
        case 40034:
