...
bool CheckMM7CD(char c) {
    ...
    swprintf(pMagicPath, wcslen(L"%C:\\anims\\magic7.vid"),
             L"%C:\\anims\\magic7.vid", c);
    if (GetFileAttributesW(pMagicPath) == -1) return false;
    ...
    *Filename = c;
    ...
    if (!fseek(f, 0, SEEK_END)) {
        if (!fseek(f, -100, SEEK_CUR)) fread(DstBuf, 1, 0x64u, f);
...
int __stdcall InsertMM7CDDialogFunc(HWND hDlg, int a2, __int16 a3, int a4) {
    ...
    if (a2 == 272) {
        ...
        if (v6) {
            ...
            if (v7) {
                ...
                if (v8) {
                    ...
                    if (v9) {
                        ...
                        SetDlgItemTextA(
                            hDlg, 1010,
                            "Wloz CD-ROM numer 2 Might and Magic® VII.");
                        v11 = "Odwolaj";
                    } else {
                        ...
                        SetDlgItemTextA(
                            hDlg, 1010,
                            "Inserire il secondo CD di Might and Magic® VII.");
                        v11 = "Annulla";
                    }
                } else {
                    SetWindowTextA(hDlg, "Insйrez le CD 2");
                    v10 = SetDlgItemTextA;
                    SetDlgItemTextA(hDlg, 1010,
                                    "Insйrez Might & Magic® VII CD 2.");
                    v11 = "Supprimer";
                }
            } else {
                ...
                SetDlgItemTextA(
                    hDlg, 1010,
                    "Por favor, inserte disco 2 de Might & Magic® VII.");
                v11 = "Cancelar";
            }
        } else {
            ...
            v10 = SetDlgItemTextA;
            SetDlgItemTextA(hDlg, 1010,
                            "Bitte CD 2 von Might and Magic® VII einlegen.");
            v11 = "Abbrechen";
        }
        v10(hDlg, 2, v11);
        return 0;
...
bool OS_FindMM7CD(char *pCDDrive) {
    ...
    HKEY hSoftware = nullptr, hNWC = nullptr, hMM7 = nullptr,
         hVersion = nullptr;
    if (!RegOpenKeyExA(HKEY_LOCAL_MACHINE, "SOFTWARE", 0,
                       KEY_READ | KEY_WOW64_32KEY, &hSoftware)) {
        if (!RegOpenKeyExA(hSoftware, "New World Computing", 0,
                           KEY_READ | KEY_WOW64_32KEY, &hNWC)) {
            if (!RegOpenKeyExA(hNWC, "Might and Magic VII", 0,
                               KEY_READ | KEY_WOW64_32KEY, &hMM7)) {
                if (!RegOpenKeyExA(hMM7, "1.0", 0, KEY_READ | KEY_WOW64_32KEY,
                                   &hVersion)) {
                    DWORD cbData = 3;
                    if (!RegQueryValueExA(hVersion, "CDDrive", 0, 0,
                                          (BYTE *)drive, &cbData))
    ...
    while (true) {
        ...
        if (DialogBoxParamA(GetModuleHandleW(nullptr), "InsertCD", nullptr,
                            (DLGPROC)InsertMM7CDDialogFunc, 0))
            continue;
        return false;
    }
}
