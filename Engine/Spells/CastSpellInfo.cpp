  ...
  for(n = 0; n < CastSpellInfoCount; ++n)
  {
    ...
    switch ( pCastSpell->uSpellID )
    {
      ...
      case SPELL_EARTH_SLOW://Замедление
      {
        switch (skill_level)
        {
          case 1: LODWORD(v733) = 180 * spell_level; amount = 2; break;//LODWORD(v733)???не применяется далее
      ...
      case SPELL_EARTH_TELEKINESIS://Телекинез
      {
        ...
        if (PID_TYPE(a2) == OBJECT_Item)
        {
          ...
          {
              sprintfex(pTmpBuf2.data(), pGlobalTXT_LocalizationStrings[471], pItemsTable->pItems[pSpriteObjects[obj_id].containing_item.uItemID].pUnidentifiedName);//Вы нашли ^Pv[%s]!
            ShowStatusBarString(pTmpBuf2.data(), 2);
