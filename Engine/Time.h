...
struct GameTime {
  ...
  uint64_t GetSeconds() const { return (uint64_t)(this->value * TIME_UNPACK_GAME_SECONDS); }
  ...
  void AddMinutes(int minutes) { this->value += (uint64_t)(60 * minutes * TIME_PACK_GAME_SECONDS); }
  void SubtractHours(int hours) { this->value -= (uint64_t)(60 * 60 * hours * TIME_PACK_GAME_SECONDS); }
