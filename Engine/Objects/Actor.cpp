...
void Actor::LootActor()
{
  ...
  else
  {
    if ( rand() % 100 < this->pMonsterInfo.uTreasureDropChance && (v7 = this->pMonsterInfo.uTreasureLevel) != 0 )
    {
      ...
      if ( v14 )
        sprintfex(pTmpBuf2.data(), pGlobalTXT_LocalizationStrings[490], v14, v10);//¬ы нашли ^I[%d] золот^L[ой;ых;ых] и предмет (%s)!
      else
        sprintfex(pTmpBuf2.data(), pGlobalTXT_LocalizationStrings[471], v10);//¬ы нашли ^Pv[%s]!
      ShowStatusBarString(pTmpBuf2.data(), 2);
      if ( !pParty->AddItemToParty(&Dst) )
        pParty->SetHoldingItem(&Dst);
      itemFound = true;
