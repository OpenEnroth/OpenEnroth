...
void OutdoorLocation::ExecDraw(unsigned int bRedraw) {
    ...
    if (engine->config->graphics.GetSpecular())
...
void ODM_ProcessPartyActions() {
    ...
    if (pParty->bFlying)  // в полёте
        ceiling_height = GetCeilingHeight(pX, pY, party_new_Z + pParty->uPartyHeight, &v102);  // высота потолка
    ...
    if (engine->config->settings.GetWalkSound() && pParty->walk_sound_timer) {
        ...
        switch (pPartyActionQueue->Next()) {
            case PARTY_FlyUp:  // полёт вверх
            {
                ...
                if (engine->IsUnderwater() ||
                    ...
                    if (pParty->vPosition.z < engine->config->gameplay.GetMaxFlightHeight() || hovering) {
                        party_new_Z += 30;
                        v113 += 30;
                        pParty->bFlying = true;
                        if (party_new_Z > engine->config->gameplay.GetMaxFlightHeight()) {
                            party_new_Z = engine->config->gameplay.GetMaxFlightHeight();
                            v113 = engine->config->gameplay.GetMaxFlightHeight();
                        }
                        v1 = 0;
                        v2 = 0;
                        fall_speed = 0;
            ...
            case PARTY_TurnLeft:  // поворот влево
                if (engine->config->settings.GetTurnSpeed() > 0)
                    _angle_y += engine->config->settings.GetTurnSpeed();  // descrete turn
                else
            ...
            case PARTY_TurnRight:  // поворот вправо
                if (engine->config->settings.GetTurnSpeed() > 0)
                    _angle_y -= engine->config->settings.GetTurnSpeed();
                else
            ...
            case PARTY_WalkForward:  // идти вперёд
            {
                *(float *)&v128 = _walk_speed;
                ...
                if (engine->config->debug.GetTurboSpeed()) {
                    v2 += dx * 12;
                    v1 += dy * 12;
                } else {
                    v2 += dx;
            ...
            case PARTY_LookUp:
                _angle_x += engine->config->settings.GetVerticalTurnSpeed();
            ...
            if (v34 < -500 && !pParty->bFlying &&
                ...
                for (int i = 0; i < 4; ++i) {
                    if (!pParty->pPlayers[i].HasEnchantedItemEquipped(ITEM_ENCHANTMENT_OF_FEATHER_FALLING) &&
                        ...
                        pParty->pPlayers[i].PlaySound(SPEECH_Falling, 0);  // крик падения
                }
            }
        }
    } else {
        ...
        if (PID_TYPE(collision_state.pid) == OBJECT_BModel) {
            ...
            if (!v129 && (pODMFace->uPolygonType != POLYGON_InBetweenFloorAndWall || v119)) {  // упёрся в столб
                ...
                if (v55 > 0) {
                    ...
                    pY = _angle_y + fixpoint_mul(pODMFace->pFacePlaneOLD.vNormal.y, v55);
                    if (!v119)
                        party_new_Z = v122 + fixpoint_mul(pODMFace->pFacePlaneOLD.vNormal.z, v55);
                }
    ...
    if (pParty->bFlying || !not_high_fall || bWaterWalk || !v122_a)
        // полёт или высокое падение или хождение по воде или
        v77 = 1;
    else
        v77 = v122 != 0;
    ...
    } else if (engine->config->settings.GetWalkSound() && pParty->walk_sound_timer <= 0) {
        pAudioPlayer->StopAll(804);
            ...
            if (pParty->uFallStartZ - party_new_Z > 512 && !bFeatherFall &&
                ...
                if (pParty->uFlags & PARTY_FLAGS_1_LANDING) {
                    ...
                    for (uint i = 1; i <= 4; ++i) {
                        ...
                        pPlayers[i]->ReceiveDamage(
                            (int)((pParty->uFallStartZ - party_new_Z) * (uint64_t)((double)v110 * 0.1)) / 256,
                            DMGT_PHISYCAL);
                        v110 = 20 - pPlayers[i]->GetParameterBonus(pPlayers[i]->GetActualEndurance());
                        pPlayers[i]->SetRecoveryTime(
                            (signed __int64)((double)v110 * debug_non_combat_recovery_mul * flt_debugrecmod3));
