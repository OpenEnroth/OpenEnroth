...
void BLVRenderParams::Reset() {
    ...
    // if ( render->pRenderD3D )
    {
        ...
        // this->fCosineY = cos((3.141592653589793 + 3.141592653589793) *
        // (double)pIndoorCameraD3D->sRotationY * 0.00048828125); this->fSineY =
        // sin((3.141592653589793 + 3.141592653589793) *
        // (double)pIndoorCameraD3D->sRotationY * 0.00048828125);
        // this->fCosineNegX = cos((3.141592653589793 + 3.141592653589793) *
        // (double)-pIndoorCameraD3D->sRotationX * 0.00048828125);
        // this->fSineNegX = sin((3.141592653589793 + 3.141592653589793) *
        // (double)-pIndoorCameraD3D->sRotationX * 0.00048828125);
    ...
    /*else
    {
      ...
      this->uViewportCenterY = this->uViewportW - ((unsigned __int64)(v26 *
    (signed __int64)v29) >> 16);
    }*/
...
void IndoorLocation::ExecDraw_d3d(unsigned int uFaceID,
    ...
    if (!pIndoorCameraD3D->IsCulled(pFace)) {
        ...
        for (uint i = 0; i < pFace->uNumVertices; ++i) {
            static_vertices_F7C228[i].vWorldPosition.x =
                pIndoor->pVertices[pFace->pVertexIDs[i]].x;
            static_vertices_F7C228[i].vWorldPosition.y =
                pIndoor->pVertices[pFace->pVertexIDs[i]].y;
            static_vertices_F7C228[i].vWorldPosition.z =
        ...
        if (!pVertices || (pEngine->pStru9Instance->_498377(
                           ...
                           uNumVerticesa)) {
            if (pIndoorCameraD3D->CalcPortalShape(
                    static_vertices_F7C228, &uNumVerticesa,
                    static_vertices_F7B628,
                    pIndoorCameraD3D->std__vector_000034_prolly_frustrum, 4,
                    false, 0) != 1 ||
                uNumVerticesa) {
                a4a = HEXRAYS_SHIWORD(Lights.uCurrentAmbientLightLevel);
                v17 =
                    (248 -
                ...
                if (Lights.uNumLightsApplied > 0 ||
                    ...
                    stru_F7B60C.face_plane.vNormal.y =
                        pFace->pFacePlane.vNormal.y;
                    stru_F7B60C.face_plane.vNormal.z =
                ...
                if (pFace->Indoor_sky())
                    ...
                    render->DrawIndoorPolygon(uNumVerticesa, pFace,
                                              PID(OBJECT_BModel, uFaceID), v17,
                                              0);
                return;
            }
        }
...
unsigned int sub_4B0E07(unsigned int uFaceID) {
    ...
    if (pIndoor->pFaces[uFaceID].uAttributes & FACE_FLOW_DIAGONAL) {
        Lights.pDeltaUV[1] -=
            offset &
            (((Texture *)pIndoor->pFaces[uFaceID].resource)->GetHeight() -
             1);  // pBitmaps_LOD->GetTexture(pIndoor->pFaces[uFaceID].uBitmapID)->uHeightMinus1;
    ...
    if (pIndoor->pFaces[uFaceID].uAttributes & FACE_FLOW_HORIZONTAL) {
        Lights.pDeltaUV[0] -=
            offset &
            (((Texture *)pIndoor->pFaces[uFaceID].resource)->GetWidth() -
             1);  // pBitmaps_LOD->GetTexture(pIndoor->pFaces[uFaceID].uBitmapID)->uWidthMinus1;
...
bool IndoorLocation::Load(const String &filename, int num_days_played,
    ...
    for (uint i = 0; i < uNumSpriteObjects; ++i) {
        if (pSpriteObjects[i].containing_item.uItemID &&
            !(pSpriteObjects[i].uAttributes & 0x0100)) {
            pSpriteObjects[i].uType =
                (SPRITE_OBJECT_TYPE)pItemsTable
            ...
            for (uint j = 0; j < pObjectList->uNumObjects; ++j)
                if (pSpriteObjects[i].uType ==
                    pObjectList->pObjects[j].uObjectID) {
...
int IndoorLocation::GetSector(int sX, int sY, int sZ) {
    ...
    for (uint i = 1; i < uNumSectors; ++i) {
        ...
        for (uint j = 0; j < v51; ++j) {
            ...
            if (pFace->uNumVertices && v54 == 1) v43[v55++] = uFaceID;
        }
    }

    v4 = v43[0];
    if (v55 == 1) return this->pFaces[v4].uSectorID;
    if (!v55) return 0;
    pSectorID = 0;
    v53 = 0xFFFFFFu;
    if (v55 > 0) {
        ...
        for (v37 = 0; v37 < v55; ++v37) {
            if (this->pFaces[v43[v37]].uPolygonType == POLYGON_Floor)
                v39 =
                    sZ - this->pVertices[*this->pFaces[v43[v37]].pVertexIDs].z;
            if (this->pFaces[v43[v37]].uPolygonType ==
                POLYGON_InBetweenFloorAndWall) {
                v39 =
                    sZ -
                    ((fixpoint_mul(this->pFaces[v43[v37]].zCalc1, (sX << 16)) +
                      fixpoint_mul(this->pFaces[v43[v37]].zCalc2, (sY << 16)) +
                      this->pFaces[v43[v37]].zCalc3 + 0x8000) >>
                     16);
            }
            if (v39 >= 0) {
                if (v39 < v53) {
        ...
        if (pSpriteObjects[i].uObjectDescID) {
            if (!(pObjectList->pObjects[pSpriteObjects[i].uObjectDescID]
                ...
                if ((pSpriteObjects[i].uType < 1000 ||
                    ...
                    v6 = stru_5C6E00->Atan2(pSpriteObjects[i].vPosition.x -
                                                pIndoorCameraD3D->vPartyPos.x,
                                            pSpriteObjects[i].vPosition.y -
                                                pIndoorCameraD3D->vPartyPos.y);
                    HEXRAYS_LOWORD(v7) = pSpriteObjects[i].uFacing;
                    v9 = ((signed int)(stru_5C6E00->uIntegerPi +
                    ...
                    if (v4->uFlags & 0x20)
                        pSpriteObjects[i].vPosition.z -=
                            (signed int)(fixpoint_mul(v4->scale._internal,
                                                      v4->hw_sprites[v9]
                                                          ->uBufferHeight) /
                                         2);
                    ...
                    if (a6) {
                        pMobileLightsStack->AddLight(
                            ...
                            pObjectList
                                ->pObjects[pSpriteObjects[i].uObjectDescID]
                    ...
                    bool visible = pIndoorCameraD3D->ViewClip(
                        pSpriteObjects[i].vPosition.x,
                        pSpriteObjects[i].vPosition.y,
                        pSpriteObjects[i].vPosition.z, &view_x, &view_y,
                        &view_z);
                    if (visible) {
                        int projected_x = 0;
                        int projected_y = 0;
                        ...
                        pBillboardRenderList[uNumBillboardsToDraw - 1]
                            .uPalette = v4->uPaletteIndex;
                        pBillboardRenderList[uNumBillboardsToDraw - 1]
                            .uIndoorSectorID = pSpriteObjects[i].uSectorID;
                        // if ( render->pRenderD3D )
                        {
                            pBillboardRenderList[uNumBillboardsToDraw - 1]
                                .fov_x = pIndoorCameraD3D->fov_x;
                            pBillboardRenderList[uNumBillboardsToDraw - 1]
                                .fov_y = pIndoorCameraD3D->fov_y;
                            pBillboardRenderList[uNumBillboardsToDraw - 1]
                                .screenspace_projection_factor_x =
                                v4->scale *
                                fixed::FromInt((int)floorf(
                                    pIndoorCameraD3D->fov_x + 0.5f)) /
                                fixed::FromInt(view_x);
                            pBillboardRenderList[uNumBillboardsToDraw - 1]
                                .screenspace_projection_factor_y =
                                v4->scale *
...
void AddBspNodeToRenderList(unsigned int node_id) {
    ...
    // if ( render->pRenderD3D )
    {
        for (uint i = 0; i < pSector->uNumNonBSPFaces; ++i)
            ...
            pBspRenderer->AddFaceToRenderList_d3d(
                node_id, pSector->pFaceIDs[i]);  // рекурсия\recursion
    }
    /*else
    {
      for (uint i = 0; i < pSector->uNumNonBSPFaces; ++i)
...
//----- (0048653D) --------------------------------------------------------
void stru149::_48653D_frustum_blv(int a2, int a3, int a4, int a5, int a6,
                                  int a7) {  // portal frustum culling
    int v8;   // edi@1
    int v9;   // eax@1
    int v16;  // ST14_4@3
    int v17;  // ST10_4@3
    ...
    if (pIndoorCameraD3D->sRotationX) {
        v16 = v8 * -pIndoorCameraD3D->vPartyPos.x +
              v29 * -pIndoorCameraD3D->vPartyPos.y;
        v17 = -65536 * pIndoorCameraD3D->vPartyPos.z;
        this->field_0_party_dir_x =
            fixpoint_mul(v16, v28) +
            fixpoint_mul((-pIndoorCameraD3D->vPartyPos.z) << 16, v9);
        this->field_4_party_dir_y = v8 * -pIndoorCameraD3D->vPartyPos.y -
                                    v29 * -pIndoorCameraD3D->vPartyPos.x;
        this->field_8_party_dir_z =
            fixpoint_mul(v17, v28) - fixpoint_mul(v16, v9);
    } else {
        this->field_0_party_dir_x = v8 * -pIndoorCameraD3D->vPartyPos.x +
                                    v29 * -pIndoorCameraD3D->vPartyPos.y;
        this->field_4_party_dir_y = v8 * -pIndoorCameraD3D->vPartyPos.y -
                                    v29 * -pIndoorCameraD3D->vPartyPos.x;
    ...
    if (pIndoorCameraD3D->sRotationX) {
        ...
        this->viewing_angle_from_west_east =
            fixpoint_mul(a4, v28) - fixpoint_mul(v19, v9);
    } else {
    ...
    if (pIndoorCameraD3D->sRotationX) {
        ...
        this->viewing_angle_from_north_south =
            fixpoint_mul(a7, v28) - fixpoint_mul(v21, v9);
    } else {
...
char DoInteractionWithTopmostZObject(int a1, int a2) {
    ...
    switch (PID_TYPE(a1)) {
        ...
        {
            if (pObjectList->pObjects[pSpriteObjects[v17].uObjectDescID]
                        .uFlags &
                    0x10 ||
                v17 >= 1000 || !pSpriteObjects[v17].uObjectDescID)
                return 1;
        ...
        case OBJECT_Actor:
            if (pActors[v17].uAIState == Dying ||
                pActors[v17].uAIState == Summoned)
                return 1;
        ...
        case OBJECT_Decoration:
            extern void DecorationInteraction(unsigned int id,
                                              unsigned int pid);
        ...
        case OBJECT_BModel:
            if (uCurrentlyLoadedLevelType == LEVEL_Outdoor) {
                ...
                if (pIndoor->pFaces[v17].uAttributes & FACE_HAS_EVENT ||
                    !pIndoor->pFaceExtras[pIndoor->pFaces[v17].uFaceExtraID]
                         .uEventID)
                    return 1;
                if (current_screen_type != SCREEN_BRANCHLESS_NPC_DIALOG)
                    EventProcessor(
                        (signed __int16)pIndoor
                            ->pFaceExtras[pIndoor->pFaces[v17].uFaceExtraID]
                            .uEventID,
                        a1, 1);
            }
            return 0;
    ...
    if ((signed int)pFace->uNumVertices > 0) {
        for (uint i = 0; i < pFace->uNumVertices; ++i) {
            pIndoorCameraD3D->ApplyViewTransform_TrueIfStillVisible_BLV(
                ...
                (fixed *)&PortalFace._view_transformed_y[i + 3], false);
        }
    }
...
int GetAlertStatus() {
    ...
    if (uCurrentlyLoadedLevelType == LEVEL_Indoor)
        ...
        result = uCurrentlyLoadedLevelType == LEVEL_Outdoor
                     ? pIndoor->dlv.field_C_alert
                     : 0;
    return result;
}
    ...
    for (uint mon_id = 0; mon_id < uNumActors; ++mon_id) {
        if ((pActors[mon_id].pMonsterInfo.uID < 121 ||
            ...
            (pActors[mon_id].pMonsterInfo.uID < 133 ||
             pActors[mon_id].pMonsterInfo.uID >
                 135) &&  // Peasant Elf FemaleA A-C
            ...
            if (uCurrentlyLoadedLevelType == LEVEL_Outdoor) {
                ...
                for (face_id; face_id < 100; ++face_id) {
                    ...
                    v19.vPosition.x = pParty->vPosition.x +
                                      fixpoint_mul(stru_5C6E00->Cos(v7), v6);
                    v8 = 0;
                    v19.uIndex = a2;
                    v19.vPosition.y = fixpoint_mul(stru_5C6E00->Sin(v7), v6) +
                                      pParty->vPosition.y;
                ...
                for (uint i = 0; i < 100; ++i) {
                    ...
                    v15 = stru_5C6E00->Cos(
                        v13 % (signed int)stru_5C6E00->uIntegerDoublePi);
                    v19.vPosition.x =
                        pParty->vPosition.x + fixpoint_mul(v15, v12);
                    v16 = stru_5C6E00->Sin(
                        v13 % (signed int)stru_5C6E00->uIntegerDoublePi);
                    v19.vPosition.y =
                        fixpoint_mul(v16, v12) + pParty->vPosition.y;
                    v19.vPosition.z = pParty->vPosition.z;
                    v19.uIndex = a2;
                    v17 = pIndoor->GetSector(v19.vPosition.x, v19.vPosition.y,
                                             pParty->vPosition.z);
                    if (v17 == v22) {
                        v18 =
                            BLV_GetFloorLevel(v19.vPosition.x, v19.vPosition.y,
...
int sub_450521_ProllyDropItemAt(int ecx0, signed int a2, int a3, int a4, int a5,
    ...
    a1.uType =
        (SPRITE_OBJECT_TYPE)pItemsTable->pItems[a1.containing_item.uItemID]
            .uSpriteID;
    v11 = 0;
        ...
        if (dword_4F5CC8_ys[i] >= v8 ^ (dword_4F5CC8_ys[i + 1] >= v8)) {
            ...
            if (!(dword_4F5D98_xs[i + 1] >= a4a && dword_4F5D98_xs[i] < a4a)) {
                if ((dword_4F5D98_xs[i + 1] < a4a && dword_4F5D98_xs[i] >= a4a)) {
                    ...
                    v25 = fixpoint_div(
                        dword_4F5D98_xs[i + 1] - dword_4F5D98_xs[i],
                        dword_4F5CC8_ys[i + 1] - dword_4F5CC8_ys[i]);
                    if (dword_4F5D98_xs[i] +
                            (fixpoint_mul(v25, (v8 - dword_4F5CC8_ys[i])
                                                   << 16) +
                                 0x8000 >>
                             16) >=
                        a4a)
