...
void DecorationList::FromFile(void *data_mm6, void *data_mm7, void *data_mm8) {
    ...
    pDecorations =
        (DecorationDesc *)malloc(uNumDecorations * sizeof(DecorationDesc));
    memcpy(pDecorations, (char *)data_mm7 + 4,
           num_mm7_decs * sizeof(DecorationDesc));
    for (uint i = 0; i < num_mm6_decs; ++i) {
        memcpy(pDecorations + num_mm7_decs + i,
               (char *)data_mm6 + 4 + i * sizeof(DecorationDesc_mm6),
    ...
    memcpy(pDecorations + num_mm6_decs + num_mm7_decs, (char *)data_mm8 + 4,
           num_mm8_decs * sizeof(DecorationDesc));
}
...
bool DecorationList::FromFileTxt(const char *Args) {
    ...
    if (fgets(&Buf, 490, v3)) {
        do {
            *strchr(&Buf, 10) = 0;
            ...
            if (v42.uPropCount && *v42.pProperties[0] != 47 &&
                v42.uPropCount >= 3)
                ++Argsa;
    ...
    for (i = fgets(&Buf, 490, File); i; i = fgets(&Buf, 490, File)) {
        *strchr(&Buf, 10) = 0;
        ...
        if (v42.uPropCount && *v42.pProperties[0] != 47 &&
            v42.uPropCount >= 3) {
            strcpy(v2->pDecorations[v2->uNumDecorations].pName,
                   v42.pProperties[1]);
            v8 = pSpriteFrameTable->FastFindSprite(
                v2->pDecorations[v2->uNumDecorations].pName);
            // v9 = v42.pProperties[2];
            v2->pDecorations[v2->uNumDecorations].uSpriteID = v8;
            strcpy(v2->pDecorations[v2->uNumDecorations].field_20,
                   v42.pProperties[2]);
            // v10 = v42.pProperties[3];
            v2->pDecorations[v2->uNumDecorations].uType = 0;
            // v11 = atoi(v42.pProperties[3]);
            // v12 = v42.pProperties[4];
            v2->pDecorations[v2->uNumDecorations].uRadius =
                atoi(v42.pProperties[3]);
            // v13 = atoi(v42.pProperties[4]);
            // v14 = v42.pProperties[5];
            v2->pDecorations[v2->uNumDecorations].uDecorationHeight =
                atoi(v42.pProperties[4]);
            // v15 = atoi(v42.pProperties[5]);
            // v16 = v42.pProperties[6];
            v2->pDecorations[v2->uNumDecorations].uLightRadius =
                atoi(v42.pProperties[5]);
            // v17 = atoi(v42.pProperties[6]);
            // v18 = v42.pProperties[7];
            v2->pDecorations[v2->uNumDecorations].uColoredLightRed =
                atoi(v42.pProperties[6]);
            // v19 = atoi(v42.pProperties[7]);
            // v20 = v42.pProperties[8];
            v2->pDecorations[v2->uNumDecorations].uColoredLightGreen =
                atoi(v42.pProperties[7]);
            // v21 = atoi(v42.pProperties[8]);
            // v22 = v42.pProperties[9];
            v2->pDecorations[v2->uNumDecorations].uColoredLightBlue =
                atoi(v42.pProperties[8]);
            v2->pDecorations[v2->uNumDecorations].uSoundID =
                atoi(v42.pProperties[9]);
                        ...
                        if (_stricmp(v27, "Invisible")) {
                            if (_stricmp(v27, "FS")) {
                                if (_stricmp(v27, "FM")) {
                                    ...
                                    v31 =
                                        (int)&v2
                                            ->pDecorations[v2->uNumDecorations]
                                            .uFlags;
                                    *(char *)v31 |= 8u;
                                }
                            } else {
                                v30 =
                                    (int)&v2->pDecorations[v2->uNumDecorations]
                                        .uFlags;
                                *(char *)v30 |= 4u;
                            }
                        } else {
                            v29 = (int)&v2->pDecorations[v2->uNumDecorations]
                                      .uFlags;
                            *(char *)v29 |= 2u;
                        }
                    } else {
...
void DecorationList::ToFile() {
    ...
    v3 = v2;
    if (!v2) Error("Unable to save ddeclist.bin!");
...
unsigned __int16 DecorationList::GetDecorIdByName(const char *pName) {
    if (pName && (signed int)this->uNumDecorations > 1) {
        for (uint uID = 1; uID < (signed int)this->uNumDecorations; ++uID) {
            if (!_stricmp(pName, this->pDecorations[uID].pName)) return uID;
        }
    }
    return 0;
}
