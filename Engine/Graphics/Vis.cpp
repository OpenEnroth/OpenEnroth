...
bool Vis::IsPolygonOccludedByBillboard(RenderVertexSoft *vertices, int num_vertices, float x, float y)
{
  ...
  // //Bounding rectangle(ќграничивающий пр€моугольник)-------------------------
  //v7 = 3.4028235e38;
  float min_x = FLT_MAX;
  //a4a = 3.4028235e38;
  float min_y = FLT_MAX;
  ...
  for (BSPModel &model : pOutdoor->pBModels) {
    ...
    for (ODMFace &face : model.pFaces) {
      if (is_part_of_selection(&face, filter)) {
        ...
        if (Intersect_Ray_Face(pRay, pRay + 1, &fDepth, &intersection, &blv_face, model.index)) {
          ...
          uint32_t pid = PID(OBJECT_BModel, face.index | (model.index << 6));
          list->AddObject(&face, VisObjectType_Face, intersection.vWorldViewPosition.x, pid);
        }
...
//----- (004C1944) --------------------------------------------------------
unsigned short Vis::PickClosestActor(int object_id, unsigned int pick_depth, int a4, int a5, int a6)
{
...
bool Vis::Intersect_Ray_Face(RenderVertexSoft *pRayStart, RenderVertexSoft *pRayEnd, float *pDepth, RenderVertexSoft *Intersection, BLVFace *pFace, signed int pBModelID)
{
  ...
  int ray_dir_x = pRayEnd->vWorldPosition.x - pRayStart->vWorldPosition.x,//calculate the direction vector of the line(вычислим вектор направлени€ линий)
...
#define EPSILON 1e-6
//c2 = n*u
  c2 = pFace->pFacePlane.vNormal.x * ray_dir_y// get length of the line(Ёто дает нам длину линии)
...
//t = -d-(n*p0)/n*u
  float t = c1 / c2;//How far is crossing the line in percent for 0 to 1( ак далеко пересечение линии в процентах от 0 до 1 )

  if (t < 0 || t > 1)
    return false;

// p(t) = p0 + tu;
  Intersection->vWorldPosition.x = pRayStart->vWorldPosition.x + t * ray_dir_y;// add the interest to the start line(прибавл€ем процент линии к линии старта)
  ...
  *pDepth = t;//Record the distance from the origin of the ray («аписываем дистанцию от начала луча)
  return true;
}
...
Vis_ObjectInfo *Vis_SelectionList::SelectionPointers(int pVisObjectType, int pid)
{
    ...
    if (this->uNumPointers > 0)
    {
        ...
        {
            if (this->object_pool[i].object_type == pVisObjectType && this->object_pool[i].object_pid == pid)
      ...
      if (reachable) {
        for (ODMFace &face : model.pFaces) {
          if (is_part_of_selection(&face, filter)) {
            ...
            int pid = PID(OBJECT_BModel, face.index | (model.index << 6));
