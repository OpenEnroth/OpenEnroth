            ...
            if ((test_byte & 0x3F) > 0) {
              ...
              do {
                ...
                *temp_dec_position |= r_mask & ((uint8_t)value << (num_g_bits + num_r_bits + num_b_bits - 8));
                temp_dec_position++;
                if (row_position == header->pitch)
                  break;
              } while (count-- != 1);
            }
          } else {
            ++row_position;
            *temp_dec_position |= r_mask & ((uint8_t)test_byte << (num_g_bits + num_r_bits + num_b_bits - 8));
            temp_dec_position++;
          }
      ...
      while (row_position <  header->pitch) {
        ...
        if ((test_byte & 0xC0) == 0xC0) {
          ...
          if ((test_byte & 0x3F) > 0) {
            ...
            do {
              *temp_dec_position |= g_mask & (uint16_t)((uint8_t)value << (num_g_bits + num_b_bits - 8));
          ...
          *temp_dec_position |= g_mask & (uint16_t)((uint8_t)test_byte << (num_g_bits + num_b_bits - 8));
          temp_dec_position++;
          ++row_position;
        }
      }
...
struct Format {
  Format(size_t bpp, uint32_t rm, uint32_t gm, uint32_t bm) : bytes(bpp/8), r(rm), g(gm), b(bm) {}
  size_t bytes;
  ColorFormat r;
  ColorFormat g;
  ColorFormat b;
...
void Encode(Format f, void *picture_data, unsigned int width, unsigned int height, void *pcx_data, int max_buff_size, unsigned int* packed_size) {
...
void PCX::Encode16(void *picture_data, unsigned int width, unsigned int height, void *pcx_data, int max_buff_size, unsigned int* packed_size) {
  Format f(16, 0xF800, 0x07E0, 0x001F);
  Encode(f, picture_data, width, height, pcx_data, max_buff_size, packed_size);
}

void PCX::Encode32(void *picture_data, unsigned int width, unsigned int height, void *pcx_data, int max_buff_size, unsigned int* packed_size) {
