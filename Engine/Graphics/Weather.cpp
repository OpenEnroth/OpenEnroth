...
void Weather::DrawSnow() {
  for (unsigned int i = 0; i < Screen_Coord.size(); ++i) {
    ...
    } else if (Screen_Coord[i].x >= ((int)viewparams->uScreen_BttmR_X - size)) {
    ...
    if (Screen_Coord[i].y >= ((int)viewparams->uScreen_BttmR_Y - size)) {
      ...
      Screen_Coord[i].x = viewparams->uScreen_topL_X + (rand() % (viewparams->uScreen_BttmR_X - viewparams->uScreen_topL_X - size));
    }
...
bool Weather::OnPlayerTurn(int dangle) {
  ...
  unsigned int screen_width = viewparams->uScreen_BttmR_X - viewparams->uScreen_topL_X;
