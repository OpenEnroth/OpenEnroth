...
void _registerItemBindings(sol::state_view &luaState, sol::table &table) {
    ...
    table["items"] = luaState.create_table_with(
        ...
        * To pass the correct filter we can send a lua function or a c++ function generated by the "filter_by" function below
        */
        "get_random_item", [](const FilteItemFunction& filter) {
            if(filter) {
                std::vector<ItemId> itemsToRandomizeOn;
                auto &&spawnableItems = allSpawnableItems();
                for (auto &&itemId : spawnableItems) {
                    if (filter(itemId)) {
        ...
        "filter_by", [](std::string_view type, int value) -> FilteItemFunction {
            /*
            * The current support is pretty limited to only the minimum level filter
            * I'm exposing this only to cover all the current debug view cases but, if required in the future,
            * we can expose this in lua in a better way
            */
            if(type == "min_level") {
                return [value](ItemId itemId) {
                    return pItemTable->pItems[itemId].uItemID_Rep_St >= value;
                };
            }
            return [](ItemId) { return true; };
        }
    );
}
