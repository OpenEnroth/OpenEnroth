...
- Merge
  - Dir ~/.openenroth
  - Masking --- b/c rename(tmp, mm7) should work even if we have tmp files in the lowercase fs.
    - Merge
      - Lowercase
        - Dir /Library/OpenEnroth/
      - RomFS
...
- Mounting
  - /saves: Merge
    - Dir /Library/OpenEnroth/<saves-whatever-the-case>
    - Dir ~/.openenroth --- loadable from here too, so you don't lose your saves.
  - /: XXX
...
This way it's only one save, always. And no FS-level copying.

BUT we still want to test what happens when /saves isn't there!

- Merge
  - Memory --- all dir creation goes here.
  - Masking --- /saves removed
    - BASE ^^^

Then we call test.useFileSystem(fsfsfs from test.fileSystem());
And it's cleared on prepareForNextTest();
