...
UNIT_TEST(NativeFileSystem, LsRoot) {
    ...
    NativeFileSystem fs1(NativePath::fromWtf8("")); // Current dir.
    ...
    NativeFileSystem fs2(NativePath::fromWtf8("this_dir_doesnt_exist")); // Non-existent dir.
    EXPECT_TRUE(fs2.ls("").empty());

    NativeFileSystem fs3(NativePath::fromWtf8("1.txt")); // Not-a-dir.
...
UNIT_TEST(NativeFileSystem, LsFile) {
    ...
    NativeFileSystem fs(NativePath::fromWtf8("")); // Current dir.
...
UNIT_TEST(NativeFileSystem, ExistsRoot) {
    ...
    NativeFileSystem fs2(NativePath::fromWtf8("this_dir_doesnt_exist"));
    EXPECT_TRUE(fs2.exists(""));

    ScopedTestFile tmp("1.txt", "");
    NativeFileSystem fs3(NativePath::fromWtf8("1.txt"));
...
UNIT_TEST(NativeFileSystem, ReadRootAsFile) {
    ...
    NativeFileSystem fs(NativePath::fromWtf8("1.txt"));
...
UNIT_TEST(NativeFileSystem, DisplayPathSymmetry) {
    ...
    NativeFileSystem fs(NativePath::fromWtf8(""));
...
UNIT_TEST(NativeFileSystem, DisplayPath) {
    ...
    NativeFileSystem fs(NativePath::fromWtf8("tmp_native_dir"));
...
UNIT_TEST(NativeFileSystem, FromNativePathIsCwdRelative) {
    ...
    auto [fs, path] = NativeFileSystem::fromNativePath(NativePath::fromWtf8("1.txt"));
    EXPECT_EQ(fs->read(path).str(), "lol");

    auto [cwdFs, cwdPath] = NativeFileSystem::fromNativePath(NativePath::fromWtf8(""));
...
UNIT_TEST(NativeFileSystem, FromNativePathNormalizes) {
    ...
    auto [fs, path] = NativeFileSystem::fromNativePath(NativePath::fromWtf8("a/../b/./c"));
    EXPECT_TRUE(path.string().ends_with("b/c"));
    EXPECT_FALSE(path.isEscaping());

    auto [rootFs, rootPath] = NativeFileSystem::fromNativePath(NativePath::fromWtf8("/../../.."));
