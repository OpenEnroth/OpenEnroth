...
#include "Library/Encoding/EncodingDetector.h"

static std::optional<TextEncoding> detect(std::string_view text) {
    EncodingDetector detector;
    detector.write(text);
    std::vector<EncodingDetectionResult> results = detector.finish();
    if (results.empty()) {
        return std::nullopt;
    } else {
