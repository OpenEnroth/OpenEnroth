...
void FallbackLogSink::write(const LogCategory &category, LogLevel level, std::string_view message) {
    // `memory_buffer` keeps 500 bytes inline, so a log line up to that long doesn't allocate. Level names go through
    // `logLevelName` and not through `toString` because this sink also runs before the serialization tables are
    // initialized.
