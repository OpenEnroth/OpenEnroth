...
LogLevel SdlLogSource::level() const {
    ...
    return translateSdlLogLevel(SDL_GetLogPriority(SDL_LOG_CATEGORY_CUSTOM));
}

void SdlLogSource::setLevel(LogLevel logLevel) {
    SDL_LogPriority assertPriority = SDL_GetLogPriority(SDL_LOG_CATEGORY_ASSERT);
    ...
    SDL_SetLogPriorities(translatePlatformLogLevel(logLevel));
    ...
    SDL_SetLogPriority(SDL_LOG_CATEGORY_ASSERT, assertPriority);
