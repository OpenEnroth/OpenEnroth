...
/* [Gerark] TODO - Commented this while I understand a bit more about our renderer
...
OpenGLUiRenderer::OpenGLUiRenderer() {
    /* Probably not needed, I'll keep it here just for reference before the clean-up
    Rml::String errorMessage;
    if (RmlGL3::Initialize(&errorMessage)) {
    ...
    */
    ...
    if (Gfx::CreateShaders(*mut_program_data)) {
        ...
        fullscreen_quad_geometry = OpenGLUiRenderer::CompileGeometry(mesh.vertices, mesh.indices);
    }
}

OpenGLUiRenderer::~OpenGLUiRenderer() {
    if (fullscreen_quad_geometry) {
        OpenGLUiRenderer::ReleaseGeometry(fullscreen_quad_geometry);
...
void OpenGLUiRenderer::setViewport(int width, int height) {
...
Rml::CompiledGeometryHandle OpenGLUiRenderer::CompileGeometry(Rml::Span<const Rml::Vertex> vertices, Rml::Span<const int> indices) {
...
void OpenGLUiRenderer::RenderGeometry(Rml::CompiledGeometryHandle handle, Rml::Vector2f translation, Rml::TextureHandle texture) {
...
void OpenGLUiRenderer::ReleaseGeometry(Rml::CompiledGeometryHandle handle) {
    Gfx::CompiledGeometryData *geometry = (Gfx::CompiledGeometryData *)handle;
...
void OpenGLUiRenderer::SetScissor(Rml::Rectanglei region, bool vertically_flip) {
...
void OpenGLUiRenderer::EnableScissorRegion(bool enable) {
...
void OpenGLUiRenderer::SetScissorRegion(Rml::Rectanglei region) {
    SetScissor(region);
}

void OpenGLUiRenderer::EnableClipMask(bool enable) {
    if (enable)
        glEnable(GL_STENCIL_TEST);
    else
        glDisable(GL_STENCIL_TEST);
...
void OpenGLUiRenderer::RenderToClipMask(Rml::ClipMaskOperation operation, Rml::CompiledGeometryHandle geometry, Rml::Vector2f translation) {
...
void OpenGLUiRenderer::DrawFullscreenQuad() {
    RenderGeometry(fullscreen_quad_geometry, {}, OpenGLUiRenderer::TexturePostprocess);
}

void OpenGLUiRenderer::DrawFullscreenQuad(Rml::Vector2f uv_offset, Rml::Vector2f uv_scaling) {
    Rml::Mesh mesh;
    Rml::MeshUtilities::GenerateQuad(mesh, Rml::Vector2f(-1), Rml::Vector2f(2), {});
    ...
    RenderGeometry(geometry, {}, OpenGLUiRenderer::TexturePostprocess);
...
static void SetBlurWeights(GLint weights_location, float sigma) {
    constexpr int kNum_weights = BLUR_NUM_WEIGHTS;
    float weights[kNum_weights];
    float normalization = 0.0f;
    for (int i = 0; i < kNum_weights; i++) {
    ...
    glUniform1fv(weights_location, (GLsizei)kNum_weights, &weights[0]);
}

void OpenGLUiRenderer::RenderBlur(float sigma, const Gfx::FramebufferData &source_destination, const Gfx::FramebufferData &temp,
...
Rml::CompiledFilterHandle OpenGLUiRenderer::CompileFilter(const Rml::String &name, const Rml::Dictionary &parameters) {
...
void OpenGLUiRenderer::ReleaseFilter(Rml::CompiledFilterHandle filter) {
...
Rml::CompiledShaderHandle OpenGLUiRenderer::CompileShader(const Rml::String &name, const Rml::Dictionary &parameters) {
...
void OpenGLUiRenderer::RenderShader(Rml::CompiledShaderHandle shader_handle, Rml::CompiledGeometryHandle geometry_handle,
...
void OpenGLUiRenderer::ReleaseShader(Rml::CompiledShaderHandle shader_handle) {
    delete reinterpret_cast<CompiledShader *>(shader_handle);
}

void OpenGLUiRenderer::BlitLayerToPostprocessPrimary(Rml::LayerHandle layer_handle) {
...
void OpenGLUiRenderer::RenderFilters(Rml::Span<const Rml::CompiledFilterHandle> filter_handles) {
...
Rml::LayerHandle OpenGLUiRenderer::PushLayer() {
...
void OpenGLUiRenderer::CompositeLayers(Rml::LayerHandle source_handle, Rml::LayerHandle destination_handle, Rml::BlendMode blend_mode,
...
void OpenGLUiRenderer::PopLayer() {
    render_layers.PopLayer();
    glBindFramebuffer(GL_FRAMEBUFFER, render_layers.GetTopLayer().framebuffer);
}

Rml::TextureHandle OpenGLUiRenderer::SaveLayerAsTexture(Rml::Vector2i dimensions) {
...
Rml::CompiledFilterHandle OpenGLUiRenderer::SaveLayerAsMaskImage() {
    BlitLayerToPostprocessPrimary(render_layers.GetTopLayerHandle());
...
void OpenGLUiRenderer::UseProgram(ProgramId program_id) {
...
int OpenGLUiRenderer::GetUniformLocation(UniformId uniform_id) const {
    return program_data->uniforms.Get(active_program, uniform_id);
}

void OpenGLUiRenderer::SubmitTransformUniform(Rml::Vector2f translation) {
...
OpenGLUiRenderer::RenderLayerStack::RenderLayerStack() {
...
Rml::LayerHandle OpenGLUiRenderer::RenderLayerStack::PushLayer() {
...
void OpenGLUiRenderer::RenderLayerStack::PopLayer() {
    RMLUI_ASSERT(layers_size > 0);
    layers_size -= 1;
}

const Gfx::FramebufferData &OpenGLUiRenderer::RenderLayerStack::GetLayer(Rml::LayerHandle layer) const {
    RMLUI_ASSERT((size_t)layer < (size_t)layers_size);
    return fb_layers[layer];
}

const Gfx::FramebufferData &OpenGLUiRenderer::RenderLayerStack::GetTopLayer() const {
    return GetLayer(GetTopLayerHandle());
}

Rml::LayerHandle OpenGLUiRenderer::RenderLayerStack::GetTopLayerHandle() const {
    RMLUI_ASSERT(layers_size > 0);
    return static_cast<Rml::LayerHandle>(layers_size - 1);
}

void OpenGLUiRenderer::RenderLayerStack::SwapPostprocessPrimarySecondary() {
    std::swap(fb_postprocess[0], fb_postprocess[1]);
}

void OpenGLUiRenderer::RenderLayerStack::BeginFrame(int new_width, int new_height) {
    RMLUI_ASSERT(layers_size == 0);
...
void OpenGLUiRenderer::RenderLayerStack::EndFrame() {
    RMLUI_ASSERT(layers_size == 1);
    PopLayer();
}

void OpenGLUiRenderer::RenderLayerStack::DestroyFramebuffers() {
...
const Gfx::FramebufferData &OpenGLUiRenderer::RenderLayerStack::EnsureFramebufferPostprocess(int index) {
