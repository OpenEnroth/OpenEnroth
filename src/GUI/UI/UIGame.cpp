...
void GameUI_DrawPartySpells() {
    for (int i = 0; i < spellBuffsAtRightPanel.size(); ++i) {
        if (pParty->pPartyBuffs[spellBuffsAtRightPanel[i]].Active()) {
            render->TexturePixelRotateDraw(pPartySpellbuffsUI_XYs[i][0] / 640.,
                                           ...
                                           party_buff_icons[i],
                                           pMiscTimer->_time.toRealtimeMilliseconds() / 20 + 20 * pPartySpellbuffsUI_smthns[i]);
        }
    }
    ...
    if (current_screen_type == SCREEN_GAME || current_screen_type == SCREEN_NPC_DIALOGUE) {
        ...
        Duration frameNum = pMiscTimer->_time * 50 / 128;
...
void GameUI_DrawPortraits() {
    ...
    for (int i = 0; i < pParty->pCharacters.size(); ++i) {
        ...
        if (pPlayer->expression == CHARACTER_EXPRESSION_TALK)
            pFrame = pPlayerFrameTable->GetFrameBy_y(&pPlayer->_expression21_frameset, &pPlayer->_expression21_animtime, pMiscTimer->_dt);
        else
    ...
    if (current_screen_type == SCREEN_GAME ||
        ...
        if (pParty->TorchlightActive()) {
            render->DrawTextureNew(
                ...
                pIconsFrameTable
                    ->GetFrame(pUIAnum_Torchlight->icon->id, pMiscTimer->_time)
                    ->GetTexture());
        }
        if (pParty->wizardEyeActive()) {
            render->DrawTextureNew(
                ...
                pIconsFrameTable
                    ->GetFrame(pUIAnim_WizardEye->icon->id, pMiscTimer->_time)
                    ->GetTexture());
        }
    }
}
