    ...
    for (int i = 0; i < pParty->pCharacters.size(); ++i) {
        ...
        if (pPlayer->IsEradicated()) {
            ...
            if (pParty->pPartyBuffs[PARTY_BUFF_INVISIBILITY].Active())
                render->DrawMasked(pPlayerPortraitsXCoords_For_PlayerBuffAnimsDrawing[i] / 640.0f, 387 / 480.0f, pPortrait, 1, colorTable.MediumGrey); // was 388
            else
        ...
        if (pPlayer->IsDead()) {
            ...
            if (pParty->pPartyBuffs[PARTY_BUFF_INVISIBILITY].Active())
                render->DrawMasked(
                    ...
                    388 / 480.0f, pPortrait, 1, colorTable.MediumGrey);
            else
                render->DrawTextureNew(
