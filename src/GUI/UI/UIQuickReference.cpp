...
void GUIWindow_QuickReference::Update() {
    ...
    for (int i = 0; i < 4; ++i) {
        ...
        pY = pFontHeight + 47;
        if (i == 0)
            pGUIWindow_CurrentMenu->DrawTextInRect(pFontArrus, {22, pFontHeight + 47}, Color(), localization->GetString(LSTR_CLASS), 60, 0);
        ...
        pGUIWindow_CurrentMenu->DrawTextInRect(pFontArrus, {pX, pY}, Color(), pTempString, 84, 0);
        pY = pFontHeight + pY;
