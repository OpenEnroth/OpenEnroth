...
void GameUI_DrawItemInfo(struct ItemGen *inspect_item) {
    ...
    if (pParty->_uActiveCharacter) {
        ...
        if (!inspect_item->IsIdentified()) {
            if (pPlayers[pParty->_uActiveCharacter]->CanIdentify(inspect_item) == 1)
                inspect_item->SetIdentified();
            ...
            if (!inspect_item->IsIdentified()) {
                ...
                if (inspect_item->GetValue() <
                    100 * (pPlayers[pParty->_uActiveCharacter]->uLevel + 5))
                    v83 = SPEECH_IndentifyItemWeak;
            }
            if (dword_4E455C) {
                pPlayers[pParty->_uActiveCharacter]->PlaySound((PlayerSpeech)(int)v83, 0);
                dword_4E455C = 0;
            }
        }
        inspect_item->UpdateTempBonus(pParty->GetPlayingTime());
        if (inspect_item->IsBroken()) {
            if (pPlayers[pParty->_uActiveCharacter]->CanRepair(inspect_item) == 1)
            ...
            if (dword_4E455C) {
                pPlayers[pParty->_uActiveCharacter]->PlaySound(v83, 0);
                dword_4E455C = 0;
            }
        }
    }
...
void MonsterPopup_Draw(unsigned int uActorID, GUIWindow *pWindow) {
    ...
    if (pPlayers[pParty->_uActiveCharacter]->GetActualSkillLevel(
            PLAYER_SKILL_MONSTER_ID)) {
        skill_points = pPlayers[pParty->_uActiveCharacter]->GetActualSkillLevel(PLAYER_SKILL_MONSTER_ID);
        skill_mastery = pPlayers[pParty->_uActiveCharacter]->GetActualSkillMastery(PLAYER_SKILL_MONSTER_ID);
    ...
    if (pActors[uActorID].uAIState != Dead &&
        ...
        pPlayers[pParty->_uActiveCharacter]->PlaySound(speech, 0);
    }

    if ((signed int)(pParty->pPlayers[pParty->_uActiveCharacter - 1]
    ...
    if (pX < 24 || pX > 455 || pY < 18 || pY > 36) {
        for (GUIButton *pButton : pGUIWindow_CurrentMenu->vButtons) {
            if (pButton->msg == UIMSG_SkillUp && pX >= pButton->uX &&
                ...
                std::string pSkillDescText = CharacterUI_GetSkillDescText(pParty->_uActiveCharacter - 1, skill);
    ...
    switch (pStringNum) {
        ...
        case 10:  // Player Condition
        {
            ...
            for (Condition condition : conditionImportancyTable()) {
                if (pPlayers[pParty->_uActiveCharacter]->conditions.Has(condition)) {
                    str += " \n";
                    GameTime condition_time =
                        pParty->GetPlayingTime() - pPlayers[pParty->_uActiveCharacter]->conditions.Get(condition);
        ...
        case 14:  // Experience
        {
            v15 = pPlayers[pParty->_uActiveCharacter]->uLevel;
            do {
                if (pPlayers[pParty->_uActiveCharacter]->uExperience < GetExperienceRequiredForLevel(v15))
                    break;
                ++v15;
            } while (v15 <= 10000);
            ...
            std::string str1;
            std::string str2;
            if (v15 > pPlayers[pParty->_uActiveCharacter]->uLevel)
                str1 = localization->FormatString(
                    LSTR_ELIGIBLE_TO_LEVELUP, v15);
            str2 = localization->FormatString(
                LSTR_XP_UNTIL_NEXT_LEVEL,
                (int)(GetExperienceRequiredForLevel(v15) - pPlayers[pParty->_uActiveCharacter]->uExperience),
                v15 + 1);
            str1 += "\n" + str2;
        ...
        case 15:  // Attack Bonus
            if (pAttackBonusAttributeDescription) {
                int meleerecov = pPlayers[pParty->_uActiveCharacter]->GetAttackRecoveryTime(false);
        ...
        case 17:  // Missle Bonus
            if (pMissleBonusAttributeDescription) {
                int missrecov = pPlayers[pParty->_uActiveCharacter]->GetAttackRecoveryTime(true);
        ...
        case 26:  // Class description
        {
            CharacterUI_DrawTooltip(localization->GetClassName(
                                        pPlayers[pParty->_uActiveCharacter]->classType),
...
void DrawSpellDescriptionPopup(int spell_index_in_book) {
    ...
    Pointi pt = mouse->GetCursorPos();
    SPELL_TYPE spell_id = static_cast<SPELL_TYPE>(spell_index_in_book + 11 * pPlayers[pParty->_uActiveCharacter]->lastOpenedSpellbookPage + 1);
    ...
    PLAYER_SKILL_TYPE skill = static_cast<PLAYER_SKILL_TYPE>(pPlayers[pParty->_uActiveCharacter]->lastOpenedSpellbookPage + 12);
    PLAYER_SKILL_MASTERY skill_mastery = pPlayers[pParty->_uActiveCharacter]->GetSkillMastery(skill);
    ...
    if (pParty->pPickedItem.uItemID != ITEM_NULL) {  // нажатие на портрет перса правой кнопкой
        ...
        for (uint i = 0; i < 4; ++i) {
            if ((signed int)pX > RightClickPortraitXmin[i] &&
                ...
                pPlayers[pParty->_uActiveCharacter]->UseItem_DrinkPotion_etc(i + 1, 1);
                return;
            }
        }
    }
...
void Inventory_ItemPopupAndAlchemy() {  // needs cleaning
    ...
    if (pX <= 13 || pX >= 462) {  // items out of inventory(вещи вне инвентаря)
        ...
        if (!ringscreenactive()) {  // rings not displayd
            ...
            item = &pPlayers[pParty->_uActiveCharacter]->pInventoryItemList[item_pid];
            ...
            for (int i = 0; i < 6; ++i) {
                if (mousex >= RingsX[i] && mousex <= (RingsX[i] + slot) &&
                    ...
                    // ring
                    // pitem = pPlayers[pParty->_uActiveCharacter]->GetNthRingItem(i);
                    // //10+i
                    pos = RingSlot(i);
                }
            }
            ...
            if (pos != ITEM_SLOT_INVALID)
                item = pPlayers[pParty->_uActiveCharacter]->GetNthEquippedIndexItem(pos);
    ...
    item = pPlayers[pParty->_uActiveCharacter]->GetItemAtInventoryIndex(invMatrixIndex);
    ...
    PLAYER_SKILL_LEVEL alchemy_skill_points = pPlayers[pParty->_uActiveCharacter]->GetActualSkillLevel(PLAYER_SKILL_ALCHEMY);
    PLAYER_SKILL_MASTERY alchemy_skill_level = pPlayers[pParty->_uActiveCharacter]->GetActualSkillMastery(PLAYER_SKILL_ALCHEMY);
    ...
    if (pParty->pPickedItem.uItemID >= ITEM_REAGENT_WIDOWSWEEP_BERRIES &&
        ...
        no_rightlick_in_inventory = 1;
        if (dword_4E455C) {
            pPlayers[pParty->_uActiveCharacter]->PlaySound(SPEECH_PotionSuccess, 0);
            dword_4E455C = 0;
        }
        return;
    }
    ...
    if (IsPotion(pParty->pPickedItem.uItemID) && IsPotion(item->uItemID)) {
        ...
        int item_pid = pPlayers[pParty->_uActiveCharacter]->GetItemListAtInventoryIndex(
            invMatrixIndex);
        // int pOut_x = item_pid + 1;
        // for (uint i = 0; i < 126; ++i)
        //{
        //  if (pPlayers[pParty->_uActiveCharacter]->pInventoryMatrix[i] == pOut_x)
        // {
        //    pOut_y = i;
        //   break;
        //}
        ...
        if (damage_level > 0) {
            pPlayers[pParty->_uActiveCharacter]->RemoveItemAtInventoryIndex(
                invMatrixIndex);  // pOut_y); ?? quickfix needs checking

            if (damage_level == 1) {
                pPlayers[pParty->_uActiveCharacter]->ReceiveDamage(grng->Random(11) + 10, DMGT_FIRE);
            } else if (damage_level == 2) {
                pPlayers[pParty->_uActiveCharacter]->ReceiveDamage(grng->Random(71) + 30, DMGT_FIRE);
                pPlayers[pParty->_uActiveCharacter]->ItemsPotionDmgBreak(1);  // break 1
            } else if (damage_level == 3) {
                pPlayers[pParty->_uActiveCharacter]->ReceiveDamage(grng->Random(201) + 50, DMGT_FIRE);
                pPlayers[pParty->_uActiveCharacter]->ItemsPotionDmgBreak(5);  // break 5
            } else if (damage_level >= 4) {
                pPlayers[pParty->_uActiveCharacter]->SetCondition(Condition_Eradicated, 0);
                pPlayers[pParty->_uActiveCharacter]->ItemsPotionDmgBreak(
                    0);  // break everything
            }
            ...
            if (dword_4E455C) {
                if (pPlayers[pParty->_uActiveCharacter]->CanAct())
                    pPlayers[pParty->_uActiveCharacter]->PlaySound(SPEECH_PotionExplode, 0);
                ...
                if (item->uItemID == ITEM_POTION_CATALYST ||
                    ...
                    item->uEnchantmentType =
                        (pParty->pPickedItem.uEnchantmentType +
                         item->uEnchantmentType) /
                        2;
                    pPlayers[pParty->_uActiveCharacter]->SetVariable(
                        VAR_AutoNotes,
                        pItemTable->potion_note[potion1_id][potion2_id]);
                }
                int bottle =
                    pPlayers[pParty->_uActiveCharacter]->AddItem(-1, ITEM_POTION_BOTTLE);
                if (bottle)
