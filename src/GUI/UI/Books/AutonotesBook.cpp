...
GUIWindow_AutonotesBook::GUIWindow_AutonotesBook() : GUIWindow_Book() {
    ...
    pBtn_Book_1 = CreateButton(pViewport->rect.topLeft() + Pointi(398, 1), {50, 34}, 1, 0,
    ...
    pBtn_Book_2 = CreateButton(pViewport->rect.topLeft() + Pointi(398, 38), {50, 34}, 1, 0,
    ...
    pBtn_Book_3 = CreateButton(pViewport->rect.topLeft() + Pointi(398, 113), {50, 34}, 1, 0,
    ...
    pBtn_Book_4 = CreateButton(pViewport->rect.topLeft() + Pointi(399, 150), {50, 34}, 1, 0,
    ...
    pBtn_Book_5 = CreateButton(pViewport->rect.topLeft() + Pointi(397, 188), {50, 34}, 1, 0,
    ...
    pBtn_Book_6 = CreateButton(pViewport->rect.topLeft() + Pointi(397, 226), {50, 34}, 1, 0,
    ...
    pBtn_Autonotes_Misc = CreateButton(pViewport->rect.topLeft() + Pointi(397, 264), {50, 34}, 1, 0,
    ...
    pBtn_Autonotes_Instructors = CreateButton(pViewport->rect.topLeft() + Pointi(397, 302), {50, 34}, 1, 0,
...
void GUIWindow_AutonotesBook::Update() {
    ...
    render->DrawQuad2D(ui_book_autonotes_background, pViewport->rect.topLeft());
    ...
    if (_bookButtonClicked && _bookButtonAction == BOOK_NOTES_MISC) {
        if (autonoteBookDisplayType == AUTONOTE_MISC) {
            render->DrawQuad2D(ui_book_button7_on, pViewport->rect.topLeft() + Pointi(397, 264));
        } else {
            pAudioPlayer->playUISound(SOUND_StartMainChoice02);
        ...
        if (autonoteBookDisplayType == AUTONOTE_MISC) {
            ...
            render->DrawQuad2D(ui_book_button7_off, pViewport->rect.topLeft() + Pointi(408, 263));
        }
    }

    if (_bookButtonClicked && _bookButtonAction == BOOK_NOTES_INSTRUCTORS) {
        if (autonoteBookDisplayType == AUTONOTE_TEACHER) {
            render->DrawQuad2D(ui_book_button8_on, pViewport->rect.topLeft() + Pointi(397, 302));
        } else {
            pAudioPlayer->playUISound(SOUND_StartMainChoice02);
        ...
        if (autonoteBookDisplayType == AUTONOTE_TEACHER) {
            ...
            render->DrawQuad2D(ui_book_button8_off, pViewport->rect.topLeft() + Pointi(408, 302));
        }
    }

    // for title
    autonotes_window.frameRect = pViewport->rect;
