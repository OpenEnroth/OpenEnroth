...
void GUIWindow_TownHall::bountyHuntingDialogueOptionClicked() {
    ...
    if (!pParty->monster_for_hunting_killed[house]) {
        ...
        // Get prize
        if (pParty->monster_id_for_hunting[house] != MONSTER_INVALID) {
            int bounty = 100 * pMonsterStats->pInfos[pParty->monster_id_for_hunting[house]].level;
...
std::string GUIWindow_TownHall::bountyHuntingText() {
    ...
    std::string name = fmt::format("{::}{}{::}", colorTable.PaleCanary.tag(), pMonsterStats->pInfos[_bountyHuntMonsterId].name, colorTable.White.tag());
    return fmt::sprintf(_bountyHuntText, name, 100 * pMonsterStats->pInfos[_bountyHuntMonsterId].level);
}
