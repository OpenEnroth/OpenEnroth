...
void GUIWindow_Bank::mainDialogue() {
    ...
    bank_window.w = SIDE_TEXT_BOX_WIDTH;
    ...
    DrawTitleText(assets->pFontArrus.get(), 0, 220, colorTable.PaleCanary, balance_str, 3, bank_window);
...
void GUIWindow_Bank::putGoldDialogue() {
    ...
    bank_window.x = SIDE_TEXT_BOX_POS_X;
    bank_window.w = SIDE_TEXT_BOX_WIDTH;
    DrawTitleText(assets->pFontArrus.get(), 0, 220, colorTable.PaleCanary,
                              fmt::format("{}: {}", localization->str(LSTR_BALANCE), pParty->uNumGoldInBank), 3, bank_window);

    if (keyboard_input_status == WINDOW_INPUT_IN_PROGRESS) {
        DrawTitleText(assets->pFontArrus.get(), 0, 146, colorTable.PaleCanary,
                                  fmt::format("{}\n{}", localization->str(LSTR_DEPOSIT), localization->str(LSTR_HOW_MUCH)), 3, bank_window);
        DrawTitleText(assets->pFontArrus.get(), 0, 186, colorTable.White, keyboardInputHandler->GetTextInput(), 3, bank_window);
        DrawFlashingInputCursor(assets->pFontArrus->GetLineWidth(keyboardInputHandler->GetTextInput()) / 2 + 80, 185, assets->pFontArrus.get(), bank_window);
        return;
    }
...
void GUIWindow_Bank::getGoldDialogue() {
    ...
    if (keyboard_input_status == WINDOW_INPUT_IN_PROGRESS) {
        DrawTitleText(assets->pFontArrus.get(), 0, 146, colorTable.PaleCanary,
                                  fmt::format("{}\n{}", localization->str(LSTR_WITHDRAW), localization->str(LSTR_HOW_MUCH)), 3, bank_window);
        DrawTitleText(assets->pFontArrus.get(), 0, 186, colorTable.White, keyboardInputHandler->GetTextInput(), 3, bank_window);
        DrawFlashingInputCursor(assets->pFontArrus->GetLineWidth(keyboardInputHandler->GetTextInput()) / 2 + 80, 185, assets->pFontArrus.get(), bank_window);
        return;
