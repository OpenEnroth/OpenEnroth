...
void GUIWindow::HouseDialogManager() {
    ...
    if (current_screen_type == CURRENT_SCREEN::SCREEN_E) {
        CharacterUI_InventoryTab_Draw(pPlayers[pParty->_uActiveCharacter], true);
        if (pDialogueNPCCount == uNumDialogueNPCPortraits && uHouse_ExitPic) {
            render->DrawTextureNew(556 / 640.0f, 451 / 480.0f,
                dialogue_ui_x_x_u);
            render->DrawTextureNew(476 / 640.0f, 451 / 480.0f,
            ...
            if (topic == DIALOGUE_79_mastery_teacher) {
                if (guild_membership_approved) {
                    ...
                    if (pParty->_uActiveCharacter) {
                        pPlayers[pParty->_uActiveCharacter]->SetSkillMastery(dword_F8B1AC_skill_being_taught, dword_F8B1B0_MasteryBeingTaught);
                        pPlayers[pParty->_uActiveCharacter]->PlaySound(SPEECH_SkillMasteryInc, 0);
                    }
                ...
                if (topic == DIALOGUE_82_join_guild && guild_membership_approved) {
                    ...
                    pCurrentFrameMessageQueue->AddGUIMessage(UIMSG_Escape, 1, 0);
                    if (pParty->_uActiveCharacter) {
                        pPlayers[pParty->_uActiveCharacter]->PlaySound(SPEECH_JoinedGuild, 0);
                        BackToHouseMenu();
                        return;
                    }
                }
...
std::string _4B254D_SkillMasteryTeacher(int trainerInfo) {
    ...
    Player *activePlayer = pPlayers[pParty->_uActiveCharacter];
...
void SeekKnowledgeElswhereDialogueOption(GUIWindow* dialogue, Player* player) {
    std::string str = SeekKnowledgeElswhereString(pPlayers[pParty->_uActiveCharacter]);
...
void SkillTrainingDialogue(
    ...
    for (int i = pDialogueWindow->pStartingPosActiveItem;
        ...
        if (pButton->msg_param == DIALOGUE_GUILD_BUY_BOOKS) {
            ...
            if (byte_4ED970_skill_learn_ability_by_class_table[pPlayers[pParty->_uActiveCharacter]->classType][skill_id] == PLAYER_SKILL_MASTERY_NONE
                || pPlayers[pParty->_uActiveCharacter]->pActiveSkills[skill_id]) {
                pButton->uW = 0;
                pButton->uHeight = 0;
                pButton->uY = 0;
            } else {
...
const char* GetJoinGuildDialogueOption(GUILD_ID guild_id) {
    ...
    if (pParty->_uActiveCharacter == 0)
        pParty->_uActiveCharacter = pParty->GetFirstCanAct();  // avoid nzi

    if (pPlayers[pParty->_uActiveCharacter]->CanAct()) {
        if (_449B57_test_bit((uint8_t*)pPlayers[pParty->_uActiveCharacter]->_achieved_awards_bits, dword_F8B1AC_award_bit_number)) {
