...
LodToolOptions LodToolOptions::parse(int argc, char **argv) {
    ...
    cat->add_option("--palettes-lod-path", result.palettesLodPath, "Path to bitmaps.lod to use for sprite palettes.")
        ->check(CLI::ExistingFile)->option_text("PATH");
    ...
    extract->add_option("--palettes-lod-path", result.palettesLodPath, "Path to bitmaps.lod to use for sprite palettes.")
        ->check(CLI::ExistingFile)->option_text("PATH");
