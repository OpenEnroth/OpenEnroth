...
static std::vector<EvtCommandInfo> makeCommands() {
    ...
    const std::vector<EvtFieldInfo> timer = {{"EachYear", EVT_FIELD_U8}, {"EachMonth", EVT_FIELD_U8}, {"EachWeek", EVT_FIELD_U8},
                                             {"Hour", EVT_FIELD_U8}, {"Minute", EVT_FIELD_U8}, {"Second", EVT_FIELD_U8},
                                             {"IntervalInHalfMinutes", EVT_FIELD_U16}, {"unk", EVT_FIELD_U16}};
    ...
    return {
        ...
        {EVENT_MouseOver, "Hint", EVT_COMMAND_STRUCTURAL, {{"Str", EVT_FIELD_U8}}},
        ...
        {EVENT_RandomGoTo, "RandomGoTo", EVT_COMMAND_STRUCTURAL,
         {{"jumpA", EVT_FIELD_U8}, {"jumpB", EVT_FIELD_U8}, {"jumpC", EVT_FIELD_U8}, {"jumpD", EVT_FIELD_U8}, {"jumpE", EVT_FIELD_U8}, {"jumpF", EVT_FIELD_U8}}},
        {EVENT_InputString, "Question", EVT_COMMAND_UNSUPPORTED, {{"Question", EVT_FIELD_I32}}},
        ...
        {EVENT_PressAnyKey, "SimpleMessage", EVT_COMMAND_UNSUPPORTED, {}},
        ...
        {EVENT_Jmp, "GoTo", EVT_COMMAND_STRUCTURAL, {jump}},
        {EVENT_OnMapReload, "OnLoadMap", EVT_COMMAND_STRUCTURAL, {unk}},
        ...
        {EVENT_OnCanShowDialogItemCmp, "CanShowTopic.Cmp", EVT_COMMAND_STRUCTURAL, variableJump},
        {EVENT_EndCanShowDialogItem, "CanShowTopic.Exit", EVT_COMMAND_STRUCTURAL, {unk}},
        {EVENT_SetCanShowDialogItem, "CanShowTopic.Set", EVT_COMMAND_STRUCTURAL, {{"Visible", EVT_FIELD_BOOL}}},
        ...
        {EVENT_CanShowTopic_IsActorKilled, "CanShowTopic.CheckMonstersKilled", EVT_COMMAND_UNSUPPORTED, monstersKilled},
        {EVENT_OnMapLeave, "OnLeaveMap", EVT_COMMAND_STRUCTURAL, {unk}},
...
struct VariableNames {
    ...
    void add(int variable, std::string name) {
        ...
        variableByName.emplace(std::move(name), variable);
    }
...
static VariableNames makeVariableNames() {
    ...
    for (std::string_view name : {"MapAlert", "BankGold", "Deaths", "MontersHunted", "PrisonTerms", "ArenaWinsPage", "ArenaWinsSquire",
                                  "ArenaWinsKnight", "ArenaWinsLord", "Invisible", "IsWearingItem"})
        result.add(variable++, std::string(name));
...
static bool decodeFields(const EvtCommandInfo &command, std::string_view payload, std::vector<EvtFieldValue> *values) {
    size_t pos = 0;
    for (const EvtFieldInfo &field : command.fields) {
        if (field.type == EVT_FIELD_STRING) {
            ...
            if (end == std::string_view::npos)
                return false;
            values->emplace_back(std::string(payload.substr(pos, end - pos)));
            pos = end + 1;
            continue;
        }
        ...
        if (field.type == EVT_FIELD_I32) {
            values->emplace_back(static_cast<int64_t>(static_cast<int32_t>(raw)));
        } else if (field.type == EVT_FIELD_MASTERY) {
            values->emplace_back(static_cast<int64_t>(raw) + 1);
        } else {
...
std::vector<EvtRecord> decodeEvtRecords(const Blob &data) {
    ...
    while (pos < bytes.size()) {
        ...
        record.payload = std::string(bytes.substr(pos + 5, size - 5));

        const EvtCommandInfo *command = evtCommand(record.opcode);
        if (!command || !decodeFields(*command, record.payload, &record.values))
            record.values.clear();
...
std::string encodeEvtRecord(const EvtRecord &record) {
    ...
    for (size_t i = 0; i < command->fields.size(); i++) {
        ...
        int size = fieldSize(field.type);
        int64_t min = field.type == EVT_FIELD_I32 ? INT32_MIN : 0; // I32 also takes unsigned values, bit masks go up to 0x80000000.
        int64_t max = (1ll << (8 * size)) - 1;
        if (raw < min || raw > max)
            throw Exception("evt.{}: value {} doesn't fit field {}", command->name, *value, field.name);
        for (int j = 0; j < size; j++)
            result.push_back(static_cast<char>((raw >> (8 * j)) & 0xFF));
    }

    if (result.size() > 256)
        throw Exception("evt.{}: the record is {} bytes long, the limit is 256", command->name, result.size());
