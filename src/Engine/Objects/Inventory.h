...
inline const Item *InventoryConstEntry::get() const {
    return _inventory ? &_inventory->_records[_index].item : nullptr;
}

InventoryZone InventoryConstEntry::zone() const {
    return _inventory ? _inventory->_records[_index].zone : INVENTORY_ZONE_STASH;
}
...
ItemSlot InventoryConstEntry::slot() const {
    return _inventory ? _inventory->_records[_index].slot : ITEM_SLOT_INVALID;
}
