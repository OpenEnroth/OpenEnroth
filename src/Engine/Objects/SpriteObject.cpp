...
int SpriteObject::Create(int yaw, int pitch, int speed, int which_char) {
    ...
    _lastParticleTime = Duration::fromTicks(pEventTimer->uTotalTimeElapsed);
...
void SpriteObject::updateObjectODM(unsigned int uLayingItemID) {
    ...
    if (!(object->uFlags & OBJECT_DESC_NO_GRAVITY)) {
        if (isAboveGround) {
            pSpriteObjects[uLayingItemID].vVelocity.z -= pEventTimer->uTimeElapsed.ticks() * GetGravityStrength();
        } else if (isHighSlope) {
            Vec3i norm;
...
void SpriteObject::updateObjectBLV(unsigned int uLayingItemID) {
    ...
    if (floor_lvl <= pSpriteObject->vPosition.z - 3) {
        pSpriteObject->vVelocity.z -= pEventTimer->uTimeElapsed.ticks() * GetGravityStrength();
