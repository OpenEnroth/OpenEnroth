 ...
 * away, so where you're calling them from becomes very important. E.g. calling `startDeterministicSegment` from an event
...
class EngineDeterministicComponent : private ProxyPlatform, private ProxyOpenGLContext {
 public:
     ...
     *
     * Note on why we're passing frame time in milliseconds here and not setting the target fps directly.
     *
     * It is possible to target e.g. 60fps, but since the platform API is in milliseconds, this will result in
