    ...
    for (int i = 0; i < ai_arrays_size; ++i) {
        ...
        if (pActors[ai_near_actors_ids[i]].CanAct()) {
            if (pActors[ai_near_actors_ids[i]].ActorNearby()) {
                ...
                v30 = v31;
                Actor::AI_StandOrBored(ai_near_actors_ids[i], Pid::character(0), 32_ticks, &v30);
    ...
    if (a_players_count > 0) {
        for (i = 0; i < a_players_count; ++i)
            players_recovery_time[i] =
                pParty->pCharacters[this->pQueue[activ_players[i]].uPackedID.id()].GetAttackRecoveryTime(false).ticks();
    ...
    for (unsigned i = 0; i < pActors.size(); ++i) {
        ...
        if (!(curr_actor->attributes & ACTOR_STAND_IN_QUEUE) &&
            ...
            if (curr_actor->currentActionTime >=
                ...
                } else if (curr_actor->aiState == InPain) {
...
void stru262_TurnBased::NextTurn() {
        ...
        if (pQueue[i].uPackedID.type() == OBJECT_Actor) {
            ...
            if ((pActors[monster_id].aiState == Dying) ||
                ...
                if (pActors[monster_id].currentActionTime <
                    ...
                    if (pActors[monster_id].aiState == InPain && !pActors[monster_id].isAirborne()) // Gets up once it lands.
    ...
    for (int i = 0; i < this->pQueue.size(); ++i) {
        if (pQueue[i].uPackedID.type() == OBJECT_Actor) {
            ...
            if ((pActors[monster_id].aiState != Dead) &&
                ...
                if (pActors[monster_id].isAirborneInPain())
                    continue;
    ...
    _494035_timed_effects__water_walking_damage__etc(213_ticks);
            ...
            if (pActors[actor_id].currentActionTime >= pActors[actor_id].currentActionLength) {
                switch (pActors[actor_id].aiState) {
                    case AttackingMelee:
                        ...
                        pushMeleeAttack(pQueue[queue_index].uPackedID, pActors[actor_id].pos + Vec3f(0, 0, pActors[actor_id].height / 2), v19);
                    ...
                    default:
                        if (!grng->random(2))
                            ...
                            Actor::AI_Stand( actor_id, ai_near_actors_targets_pid[actor_id], 64_ticks, &a4);
                }
            }
        }
    }
...
void stru262_TurnBased::AI_Action_(int queue_index) {
    ...
    if (pQueue[queue_index].uPackedID.type() == OBJECT_Actor) {
        actor_id = pQueue[queue_index].uPackedID.id();
        if (pActors[actor_id].isAirborneInPain()) {
        ...
        if (!(pActors[actor_id].aiState == Dying ||
            ...
            if (pActors[actor_id].monsterInfo.hostilityType == HOSTILITY_LONG && v22 &&
                ...
                if ((double)v9 < meleeRange) {
                    ...
                    Actor::AI_Stand(actor_id, v22, 64_ticks, &v18);
                }
...
void stru262_TurnBased::ActorAISetMovementDecision() {
    ...
    for (i = 0; i < this->pQueue.size(); ++i) {
        if (pQueue[i].uPackedID.type() == OBJECT_Actor) {
            ...
            if (pActors[pQueue[i].uPackedID.id()].isAirborneInPain())
                continue;
            if (!ActorMove(i))
    ...
    for (int i = 0; i < this->pQueue.size(); ++i) {
        if (pQueue[i].uPackedID.type() == OBJECT_Actor) {
            ...
            if (!(pActors[monster_id].buffs[ACTOR_BUFF_STONED].Active() ||
                  ...
                  pActors[monster_id].aiState == Removed ||
                  pActors[monster_id].aiState == Disabled ||
                  pActors[monster_id].isAirborneInPain())) {
        ...
        if (pQueue[i].uPackedID.type() == OBJECT_Actor) {
            ...
            if (!(curr_acror->aiState == Summoned ||
                ...
                if (curr_acror->currentActionTime >
                    ...
                    if (grng->random(2))
                        Actor::AI_Stand(uActorID, target_pid, 64_ticks, &a4);
                    else
