...
struct Party {
    ...
    /**
     * Creates an identified item and puts it into the party's hand, see `setHoldingItem`. The item is generated as
     * `ITEM_SOURCE_SCRIPT`, see `Item::postGenerate`. A spawnable artifact is marked as found.
     *
     * @param itemId                    Item type to create.
     */
    void createHoldingItem(ItemId itemId);
