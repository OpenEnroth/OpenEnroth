...
int IndoorLocation::GetSector(const Vec3f &pos) {
    ...
    for (unsigned i = 1; i < pSectors.size(); ++i) {
        ...
        if (!pSector->pBounding.intersectsCuboid(pos, Vec3f(5, 5, 64)))
        ...
        for (unsigned z = 0; z < FloorsAndPortals; ++z) {
            ...
            if (pFace->Contains(Vec3f(pos.x, pos.y, 0), MODEL_INDOOR, engine->config->gameplay.FloorChecksEps.value(), FACE_XY_PLANE))
    ...
    if (!NumFoundFaceStore) {
        if (!backupboundingsector) {
            logger->warning("GetSector fail: {}, {}, {}", pos.x, pos.y, pos.z);
            return 0;
        } else {
    ...
    float MinZDist = INT32_MAX, backupDist = INT32_MAX;
    if (NumFoundFaceStore > 0) {
        float CalcZDist = MinZDist;
        for (int s = 0; s < NumFoundFaceStore; ++s) {
            ...
            if (this->pFaces[FoundFaceStore[s]].uPolygonType == POLYGON_Floor)
                CalcZDist = pos.z - this->pVertices[*this->pFaces[FoundFaceStore[s]].pVertexIDs].z;
            if (this->pFaces[FoundFaceStore[s]].uPolygonType == POLYGON_InBetweenFloorAndWall) {
                CalcZDist = pos.z - this->pFaces[FoundFaceStore[s]].zCalc.calculate(pos.x, pos.y);
            }
...
bool BLVFace::Contains(const Vec3f &pos, int model_idx, float slack, FaceAttributes override_plane) const {
    assert(!override_plane ||
            override_plane == FACE_XY_PLANE || override_plane == FACE_YZ_PLANE || override_plane == FACE_XZ_PLANE);
...
//----- (0046CEC3) --------------------------------------------------------
int BLV_GetFloorLevel(const Vec3f &pos, int uSectorID, int *pFaceID) {
...
void BLV_ProcessPartyActions() {  // could this be combined with odm process actions?
    ...
    int faceId = -1;
    int floorZ = GetIndoorFloorZ(pParty->pos + Vec3f(0, 0, pParty->radius), &sectorId, &faceId);
...
int GetIndoorFloorZ(const Vec3f &pos, int *pSectorID, int *pFaceID) {
    if (*pSectorID != 0) {
...
//----- (0047272C) --------------------------------------------------------
int GetApproximateIndoorFloorZ(const Vec3f &pos, int *pSectorID, int *pFaceID) {
