...
GraphicsImage *BLVFace::GetTexture() {
    if (this->IsTextureFrameTable())
        ...
        return pTextureFrameTable->GetFrameTexture(
            (int64_t)this->resource, pEventTimer->_time);
    else
...
void BLV_UpdateDoors() {
    ...
    for (unsigned i = 0; i < pIndoor->pDoors.size(); ++i) {
        ...
        door->uTimeSinceTriggered += pEventTimer->_dt;
        ...
        if (actor.pos.z <= floorZ) {
            ...
            if (pIndoor->pFaces[uFaceID].uPolygonType == POLYGON_Floor) {
                ...
                if (pIndoor->pFaces[uFaceID].facePlane.normal.z < 0.68664550781f) // was 45000 fixpoint
                    actor.speed.z -= pEventTimer->_dt.ticks() * GetGravityStrength();
            }
        } else {
            if (isAboveGround && !isFlying)
                actor.speed.z += -8 * pEventTimer->_dt.ticks() * GetGravityStrength();
        }
...
void IndoorLocation::PrepareDecorationsRenderList_BLV(unsigned int uDecorationID, int uSectorID) {
    ...
    Duration v37 = pEventTimer->_time;
    if (pParty->bTurnBasedModeOn) v37 = pMiscTimer->_time;
...
void BLV_ProcessPartyActions() {  // could this be combined with odm process actions?
    ...
    int rotation =
        pEventTimer->_dt.ticks() * pParty->_yawRotationSpeed * TrigLUT.uIntegerPi / 180 / Duration::TICKS_PER_REALTIME_SECOND;
    ...
    if (isAboveGround) {
        pParty->speed.z += -2.0f * pEventTimer->_dt.ticks() * GetGravityStrength();
        ...
        if (pIndoor->pFaces[faceId].facePlane.normal.z < 0.5) {
            pParty->speed.z -= 1.0f * pEventTimer->_dt.ticks() * GetGravityStrength();
        } else {
    ...
    int64_t elapsed_time_bounded = std::min(pEventTimer->_dt.ticks(), static_cast<std::int64_t>(10000));
