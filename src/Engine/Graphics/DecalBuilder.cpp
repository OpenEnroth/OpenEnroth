...
float Decal::Fade_by_time() {
    ...
    // splats fade
    Duration delta = fadetime - pEventTimer->uTotalTimeElapsed;
...
bool DecalBuilder::ApplyBloodSplatToTerrain(TILE_DESC_FLAGS terrainFlags, Vec3f *terrnorm, float *tridotdist,
        ...
        if (planedist <= bloodsplat_container->pBloodsplats_to_apply[whichsplat].radius) {
            ...
            if (terrainFlags & TILE_DESC_WATER || terrainFlags & TILE_DESC_WATER_2) {
                ...
                if (!(bloodsplat_container->pBloodsplats_to_apply[whichsplat].blood_flags & DecalFlagsFade)) {
                    ...
                    bloodsplat_container->pBloodsplats_to_apply[whichsplat].fade_timer = pEventTimer->uTotalTimeElapsed;
                }
            }
