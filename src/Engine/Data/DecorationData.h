...
struct DecorationData {
    ...
    DecorationDataFlags uFlags;
    SoundId uSoundID = SOUND_Invalid;
    Color uColoredLight;

    bool canMoveThrough() const {
        return uFlags & (DECORATION_DATA_MOVE_THROUGH | DECORATION_DATA_DONT_DRAW);
    }
    bool dontDraw() const { return uFlags & DECORATION_DATA_DONT_DRAW; }
    bool soundOnDawn() const {
        return uFlags & DECORATION_DATA_SOUND_ON_DAWN;
    }
    bool soundOnDusk() const {
        return uFlags & DECORATION_DATA_SOUND_ON_DUSK;
    }
};
