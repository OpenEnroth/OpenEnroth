...
void GUIWindow_Chest::Update() {
  if (current_screen_type == SCREEN_CHEST_INVENTORY) {
    ...
    render->ClearZBuffer(0, 479);
    int chestBitmapId = pChests[uChestID].uChestBitmapID;
    ...
    for (int item_counter = 0; item_counter < chestWidthCells * chestHeghtCells; ++item_counter) {
      ...
      if (chest_item_index > 0) {
        ...
        if (itemPixelHeght < 14)
          itemPixelHeght = 14;
        int itemPixelPosX = chest_offs_x + 32 * (item_counter % chestWidthCells) + ((int)(v13 - itemPixelWidth) / 2);
        int itemPixelPosY = chest_offs_y + 32 * (item_counter / chestHeghtCells) +
          ((int)(((itemPixelHeght - 14) & 0xFFFFFFE0) + 32 - item_texture->GetHeight()) / 2);
