...
void CharacterUI_DrawTooltip(const char *title, String &content)
{
  ...
  popup_window.uFrameHeight = pFontSmallnum->CalcTextHeight(content, &popup_window, 24) + 2 * pFontLucida->GetFontHeight() + 24;
...
void GameUI_DrawItemInfo(struct ItemGen* inspect_item)
{
    ...
    if (uActiveCharacter)
    {
        ...
        if (!inspect_item->IsIdentified())
        {
            ...
            v83 = SPEECH_9;
            if (!inspect_item->IsIdentified())
                GameUI_StatusBar_OnEvent(localization->GetString(446)); // Identify Failed
            else
            {
                v83 = SPEECH_8;
        ...
        if (inspect_item->IsBroken())
        {
            ...
            if (!inspect_item->IsBroken())
                ...
                GameUI_StatusBar_OnEvent(localization->GetString(448)); // Repair Failed
            if (dword_4E455C)
            {
    ...
    String str = localization->FormatString(463, pItemsTable->pItems[inspect_item->uItemID].pUnidentifiedName); //"Type: %s"
    ...
    switch (inspect_item->GetItemEquipType())
    {
        ...
        {
            sprintf(out_text + 100, "%s: +%d   %s: %dd%d", localization->GetString(18), // Attack
                (int)inspect_item->GetDamageMod(), localization->GetString(53),
    ...
    case EQUIP_BOW:
        sprintf(out_text + 100, "%s: +%d   %s: %dd%d", localization->GetString(203), //"Shoot"
            (int)inspect_item->GetDamageMod(), localization->GetString(53), //"Damage"
    ...
    if (!v77)
    {
        if (inspect_item->GetItemEquipType() == EQUIP_POTION)  //this is CORRECT! do not move to switch!
        {
            if (inspect_item->uEnchantmentType)
                sprintf(out_text + 200, "%s: %d", localization->GetString(449), inspect_item->uEnchantmentType); //"Power"
        }
        else if (inspect_item->GetItemEquipType() == EQUIP_REAGENT)
            sprintf(out_text + 200, "%s: %d", localization->GetString(449), inspect_item->GetDamageDice()); //"Power"
        else if (inspect_item->uEnchantmentType)
            sprintf(out_text + 200, "%s: %s +%d", localization->GetString(210), pItemsTable->pEnchantments[inspect_item->uEnchantmentType - 1].pBonusStat, inspect_item->m_enchantmentStrength); //"Special"
        else  if (inspect_item->special_enchantment)
            sprintf(out_text + 200, "%s: %s", localization->GetString(210), pItemsTable->pSpecialEnchantments[inspect_item->special_enchantment - 1].pBonusStatement);
        else if (inspect_item->uNumCharges)
            sprintf(out_text + 200, "%s: %lu", localization->GetString(464), inspect_item->uNumCharges); //"Charges"
    }
    iteminfo_window.uFrameWidth -= 12;
    iteminfo_window.uFrameZ = iteminfo_window.uFrameX + iteminfo_window.uFrameWidth - 1;
    iteminfo_window.uFrameW = iteminfo_window.uFrameY + iteminfo_window.uFrameHeight - 1;
    Str = (char *)(3 * (pFontArrus->GetFontHeight() + 8));
    v84 = &out_text[0];
    for (uint i = 1; i <= 3; i++)
    {
        if (*v84)
    ...
    if (inspect_item->uAttributes & ITEM_TEMP_BONUS && (inspect_item->special_enchantment || inspect_item->uEnchantmentType))
        iteminfo_window.uFrameHeight += pFontComic->GetFontHeight();
    v85 = 0;
    if (pFontArrus->uFontHeight)
    {
        iteminfo_window.uFrameWidth -= 24;
        if (pFontArrus->CalcTextHeight(inspect_item->GetIdentifiedName(), &iteminfo_window, 0) / (signed int)pFontArrus->uFontHeight)
    ...
    if (v77)
    {
        ...
        iteminfo_window.DrawText(pFontComic, 100, iteminfo_window.uFrameHeight - pFontComic->GetFontHeight(), 0, txt, 0, 0, 0);
        render->ResetUIClipRect();
    }
    else
    {
        if ((inspect_item->uAttributes & ITEM_TEMP_BONUS) && (inspect_item->special_enchantment || inspect_item->uEnchantmentType))
        {
            ...
            if ((((v67.field_14_exprie_month || Str) &&
                ((txt4 += StringPrintf(" %d:mo", v67.field_14_exprie_month), v67.field_14_exprie_month) || Str)
                || v67.field_C_expire_day)
                && ((txt4 += StringPrintf(" %d:dy", v67.field_C_expire_day), v67.field_14_exprie_month) || Str || v67.field_C_expire_day)
                || v67.field_8_expire_hour)
                && ((txt4 += StringPrintf(" %d:hr", v67.field_8_expire_hour), v67.field_14_exprie_month) || Str || v67.field_C_expire_day || v67.field_8_expire_hour)
                || v67.field_4_expire_minute)
            {
                txt4 += StringPrintf(" %d:mn", v67.field_4_expire_minute);
            }
            iteminfo_window.DrawText(pFontComic, 100, iteminfo_window.uFrameHeight - 2 * pFontComic->GetFontHeight(), 0, txt4.data(), 0, 0, 0);
        }

        auto txt2 = StringPrintf("%s: %lu", localization->GetString(465), inspect_item->GetValue());
        iteminfo_window.DrawText(pFontComic, 100, iteminfo_window.uFrameHeight - pFontComic->GetFontHeight(), 0, txt2.data(), 0, 0, 0);
...
void MonsterPopup_Draw(unsigned int uActorID, GUIWindow *pWindow)
{
    ...
    if (!for_effects && false)
        pWindow->DrawText(pFontSmallnum, 28, pFontSmallnum->GetFontHeight() + 193, Color16(0xE1u, 255, 0x9Bu), localization->GetString(630), 0, 0, 0);//?
    else
    {
        pText = "";
        pTextHeight = pFontSmallnum->GetFontHeight() + 193;
        for (uint i = 1; i <= 21; ++i)
        {
    ...
    if (normal_level)
    {
        ...
        pTextHeight = v106.uViewportY + pFontSmallnum->GetFontHeight() - 3;
    ...
    pTextHeight = pTextHeight + pFontSmallnum->GetFontHeight() - 6 + pFontSmallnum->GetFontHeight();
    ...
    if (expert_level)
    {
        ...
        pTextHeight = pTextHeight + pFontSmallnum->GetFontHeight() - 3;
    ...
    if (pParty->pPartyBuffs[PARTY_BUFF_DETECT_LIFE].Active())
    {
        ...
        pWindow->DrawTitleText(pFontSmallnum, 0, pWindow->uFrameHeight - pFontSmallnum->GetFontHeight() - 12, 0, txt9, 3);
    }
}
...
String CharacterUI_GetSkillDescText(unsigned int uPlayerID, PLAYER_SKILL_TYPE uPlayerSkillType)
{
    ...
    if (pFontSmallnum->GetLineWidth(localization->GetString(433)) > (signed int)v35)
        v35 = pFontSmallnum->GetLineWidth(localization->GetString(433));// Expert
    if (pFontSmallnum->GetLineWidth(localization->GetString(432)) > (signed int)v35)
        v35 = pFontSmallnum->GetLineWidth(localization->GetString(432));// Master
    if (pFontSmallnum->GetLineWidth(localization->GetString(96)) > (signed int)v35)
        v35 = pFontSmallnum->GetLineWidth(localization->GetString(96));// Grand
...
void  CharacterUI_StatsTab_ShowHint()
{
    ...
    {
        if (pt.x >= stat_string_coord[pStringNum].x && pt.x <= stat_string_coord[pStringNum].x + stat_string_coord[pStringNum].width)
        {
            if (pt.y >= stat_string_coord[pStringNum].y && pt.y <= stat_string_coord[pStringNum].y + stat_string_coord[pStringNum].height)
                break;
        }
    }
    ...
    switch (pStringNum)
    {
        ...
        case 7:// Health Points
            if (pHealthPointsAttributeDescription)
                CharacterUI_DrawTooltip(localization->GetString(108), pHealthPointsAttributeDescription);
            break;
        case 8:// Spell Points
            if (pSpellPointsAttributeDescription)
                CharacterUI_DrawTooltip(localization->GetString(212), pSpellPointsAttributeDescription);
            break;
        case 9:// Armor Class
            if (pArmourClassAttributeDescription)
                CharacterUI_DrawTooltip(localization->GetString(12), pArmourClassAttributeDescription);
            break;
        case 10:// Player Condition
        {
            ...
            for (uint i = 0; i < 18; ++i)
            {
                if (pPlayers[uActiveCharacter]->conditions_times[pConditionImportancyTable[i]].Valid())
                {
                    ...
                    if (!pDay || (pDayWord = localization->GetString(56), pDay > 1))
                        pDayWord = localization->GetString(57);
                    str += StringPrintf("%lu %s, %lu %s", pDay, pDayWord, pHour, pHourWord);
                }
            }
        ...
        case 11:// Fast Spell
            if (pFastSpellAttributeDescription)
                CharacterUI_DrawTooltip(localization->GetString(172), pFastSpellAttributeDescription);
            break;

        case 12:// Player Age
            if (pPlayerAgeAttributeDescription)
                CharacterUI_DrawTooltip(localization->GetString(5), pPlayerAgeAttributeDescription);
            break;

        case 13:// Player Level
            if (pPlayerLevelAttributeDescription)
                CharacterUI_DrawTooltip(localization->GetString(131), pPlayerLevelAttributeDescription);
            break;
        ...
        case 14://Experience
        {
            ...
            str2 = localization->FormatString(538, GetExperienceRequiredForLevel(v15) - (int)pPlayers[uActiveCharacter]->uExperience, v15 + 1);
            str1 += "\n" + str2;
        ...
        case 15:// Attack Bonus
            if (pAttackBonusAttributeDescription)
                CharacterUI_DrawTooltip(localization->GetString(587), pAttackBonusAttributeDescription);
            break;

        case 16:// Attack Damage
            if (pAttackDamageAttributeDescription)
                CharacterUI_DrawTooltip(localization->GetString(588), pAttackDamageAttributeDescription);
            break;

        case 17:// Missle Bonus
            if (pMissleBonusAttributeDescription)
                CharacterUI_DrawTooltip(localization->GetString(589), pMissleBonusAttributeDescription);
            break;

        case 18:// Missle Damage
            if (pMissleDamageAttributeDescription)
                CharacterUI_DrawTooltip(localization->GetString(590), pMissleDamageAttributeDescription);
            break;

        case 19:// Fire Resistance
            if (pFireResistanceAttributeDescription)
                CharacterUI_DrawTooltip(localization->GetString(87), pFireResistanceAttributeDescription);
            break;

        case 20:// Air Resistance
            if (pAirResistanceAttributeDescription)
                CharacterUI_DrawTooltip(localization->GetString(6), pAirResistanceAttributeDescription);
            break;

        case 21:// Water Resistance
            if (pWaterResistanceAttributeDescription)
                CharacterUI_DrawTooltip(localization->GetString(240), pWaterResistanceAttributeDescription);
            break;

        case 22:// Earth Resistance
            if (pEarthResistanceAttributeDescription)
                CharacterUI_DrawTooltip(localization->GetString(70), pEarthResistanceAttributeDescription);
            break;

        case 23:// Mind Resistance
            if (pMindResistanceAttributeDescription)
                CharacterUI_DrawTooltip(localization->GetString(142), pMindResistanceAttributeDescription);
            break;

        case 24:// Body Resistance
            if (pBodyResistanceAttributeDescription)
                CharacterUI_DrawTooltip(localization->GetString(29), pBodyResistanceAttributeDescription);
            break;

        case 25: // Skill Points
            if (pSkillPointsAttributeDescription)
                CharacterUI_DrawTooltip(localization->GetString(207), pSkillPointsAttributeDescription);
            break;
        ...
        case 26: // Class description
        {
            CharacterUI_DrawTooltip(
                ...
                localization->GetClassDescription(pPlayers[uActiveCharacter]->classType)
            );
        }
        break;
...
void  DrawSpellDescriptionPopup(int spell_index)
{
    ...
    if (pFontSmallnum->GetLineWidth(localization->GetString(433)) > v5) // Expert
        v5 = pFontSmallnum->GetLineWidth(localization->GetString(433)); // Expert
    ...
    String str = StringPrintf(
        "%s\n\n%s\t%03d:\t%03d%s\t000\n%s\t%03d:\t%03d%s\t000\n%s\t%03d:\t%03d%s\t000\n%s\t%03d:\t%03d%s",
        spell->pDescription,
        localization->GetString(431), v5 + 3, v5 + 10, spell->pBasicSkillDesc, // Normal
    ...
    spell_info_window.DrawTitleText(pFontComic, 12, 75, 0, localization->GetSkillName(pPlayers[uActiveCharacter]->lastOpenedSpellbookPage + 12), 3);

    auto str2 = StringPrintf("%s\n%d", localization->GetString(522), // SP Cost
        pSpellDatas[spell_index + 11 * pPlayers[uActiveCharacter]->lastOpenedSpellbookPage + 1].mana_per_skill[skill_level - 1]);
    spell_info_window.DrawTitleText(pFontComic, 12, spell_info_window.uFrameHeight - pFontComic->GetFontHeight() - 16, 0, str2, 3);
			...
			/*void CharacterUI_DrawTooltip(const char *title, String &content)
{
  ...
  popup_window.DrawText(pFontSmallnum, 1, pFontLucida->GetFontHeight(), 0, content, 0, popup_window.uFrameY + popup_window.uFrameHeight, 0);
            ...
            popup_window.uFrameX = 128;
            popup_window.uFrameY = 40;
            popup_window.uFrameHeight = pFontSmallnum->CalcTextHeight(pHint, &popup_window, 24) + 2 * pFontLucida->GetFontHeight() + 24;
            popup_window.uFrameZ = popup_window.uFrameX + popup_window.uFrameWidth - 1;
            popup_window.uFrameW = popup_window.uFrameY + popup_window.uFrameHeight - 1;
            popup_window.DrawMessageBox(0);
            popup_window.uFrameX += 12;
