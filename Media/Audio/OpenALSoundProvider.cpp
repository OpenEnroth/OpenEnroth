...
OpenALSoundProvider::StreamingTrackBuffer *OpenALSoundProvider::CreateStreamingTrack16(int num_channels, int sample_rate, int bytes_per_sample) {
...
void OpenALSoundProvider::Stream16(StreamingTrackBuffer *buffer, int num_samples, const void *samples, bool wait) {
  int bytes_per_sample = 2;

  int num_processed_buffers = 0;
  alGetSourcei(buffer->source_id, AL_BUFFERS_PROCESSED, &num_processed_buffers);
  if (num_processed_buffers > 0) {
    unsigned int *processed_buffer_ids = new unsigned int[num_processed_buffers];
    alSourceUnqueueBuffers(buffer->source_id, num_processed_buffers, processed_buffer_ids);
    if (CheckError()) {
...
OpenALSoundProvider::TrackBuffer *OpenALSoundProvider::CreateTrack16(int num_channels, int sample_rate, const void *data, size_t size) {
  ALenum sound_format;
  switch (num_channels) {
    case 1: {
...
void OpenALSoundProvider::PlayTrack16(TrackBuffer *buffer, bool loop, bool wait) {
  int status;
  alGetSourcei(buffer->source_id, AL_SOURCE_STATE, (int *)&status);
  if (status == AL_PLAYING) {
    return;
...
class CallBackTimer {
 public:
  ...
  bool IsRunning() const noexcept {
    return (bRunning.load(std::memory_order_acquire) && theThread.joinable());
  }
...
void AudioTrackS16::Update() {
  ...
  while (uiReservedData < uiReservedDataMinimum) {
    ...
    alBufferData(al_buffer, al_format, buffer->get_data(), buffer->get_size(), al_sample_rate);
    if (CheckError()) {
      Close();
      return;
    }
...
bool AudioSample16::Open(PAudioDataSource data_source) {
  ...
  switch (num_channels) {
    ...
    default:
      if (alIsExtensionPresent("AL_EXT_MCFORMATS")) {
        switch (num_channels) {
          case 4: al_format = alGetEnumValue("AL_FORMAT_QUAD16");  break;
          case 6: al_format = alGetEnumValue("AL_FORMAT_51CHN16"); break;
          case 7: al_format = alGetEnumValue("AL_FORMAT_61CHN16"); break;
          case 8: al_format = alGetEnumValue("AL_FORMAT_71CHN16"); break;
        }
      }
      Error("Unsupported number of audio channels: %u", num_channels);
  }
