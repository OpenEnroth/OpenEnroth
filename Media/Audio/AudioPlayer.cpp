...
void AudioPlayer::MusicPlayTrack(MusicID eTrack) {
  ...
  if (!bNoSound && bPlayerReady && uMusicVolimeMultiplier) {
    ...
    if (pCurrentMusicTrack) {
      ...
      pCurrentMusicTrack->SetVolume(pSoundVolumeLevels[uMusicVolimeMultiplier]);
...
{
  if (!bPlayerReady || (uSoundVolumeMultiplier == 0) || (eSoundID == SOUND_Invalid)) {
    return;
  }
...
PMemBuffer AudioPlayer::LoadSound(const std::string &pSoundName) {
  ...
  if (header->uCompressedSize >= header->uDecompressedSize) {
    ...
    if (header->uDecompressedSize) {
      NumberOfBytesRead = fread((void*)buffer->get_data(), 1, header->uDecompressedSize, hAudioSnd);
    } else {
      logger->Warning(L"Can't load sound file!");
    }
  } else {
    PMemBuffer compressed = AllocMemBuffer(header->uCompressedSize);
    NumberOfBytesRead = fread((void*)compressed->get_data(), 1, header->uCompressedSize, hAudioSnd);
    zlib::Uncompress((void*)buffer->get_data(), &header->uDecompressedSize,
                     (void*)compressed->get_data(), header->uCompressedSize);
  }
