...
GAME_TEST(Issues, Issue783) {
    ...
    test.playTraceFromTestData("issue_783.mm7", "issue_783.json", [&] {
        ...
        for (CharacterBuff buff : allPotionBuffs())
            EXPECT_GT(pParty->pCharacters[0].pCharacterBuffs[buff].GetExpireTime(), pParty->GetPlayingTime() + Time::fromHours(10));
    });

    EXPECT_GT(timeTape.delta(), Time::fromHours(8)); // Check that we did rest.
    EXPECT_LT(timeTape.delta(), Time::fromHours(10)); // Check that we didn't wait out the buff expire times.
...
GAME_TEST(Issues, Issue895) {
    ...
    EXPECT_LT(timeTape.delta(), Time::fromMinutes(5));
}
