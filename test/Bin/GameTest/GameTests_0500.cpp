...
GAME_TEST(Issues, Issue518) {
    ...
    EXPECT_EQ(midairTape.max(), 0); // The stops were actors standing up in mid-air once their stun timer ran out.
...
GAME_TEST(Issues, Issue987) {
    ...
    auto expectGrandmasterTrainingAvailable = [](Class classType, Skill skill) {
        ...
        ASSERT_EQ(handleScriptedNPCTopicSelection(DIALOGUE_SCRIPTED_LINE_1, &teacher), DIALOGUE_MASTERY_TEACHER_OFFER);
